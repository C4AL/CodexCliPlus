...
相对直接运行原版 CLIProxyAPI（CPA），CodexCliPlus 当前只列以下已经落地的桌面化、本地化、Codex 专向和移植增强能力：

- **Windows 桌面宿主**：WPF + WebView2 主窗口、系统托盘、关闭最小化、桌面通知、外部链接转交系统浏览器，以及 WebView2、WebUI 或后端不可用时的原生阻断视图。
- **托管后端运行时**：把 CPA 后端作为 CodexCliPlus 管理的 `ccp-core.exe` 资产，负责资产准备、运行时配置写入、启动/停止/重启、管理密钥注入和健康状态跟踪。
- **内置本地 WebUI**：从仓库内置 WebUI 构建并随应用发布，通过本地 `assets/webui/upstream/dist/index.html` 加载，不依赖远程管理页面。
- **Codex 专向管理面**：管理入口聚焦 Codex CLI，检测并修复 Node.js、npm、PowerShell、PATH、WSL、Codex CLI 等本地依赖，并保留 Codex 账号、配置、日志和用量入口。
...
- **GPT/Codex 专用后端裁剪**：保留 OpenAI/Codex 兼容入口和 `/backend-api/codex` 直连别名，禁用不属于 CodexCliPlus 当前定位的 Gemini 兼容 API 路由。
- **本机凭据保护**：首次启动生成本地安全密钥；管理密钥和敏感值通过 Windows DPAPI 保存；API Key、OAuth Token、Authorization、Cookie 等写入时迁移为 `ccp-secret://` 引用。
- **安全账号包导入导出**：含凭据配置只允许导出为 `.sac v2` 加密包，使用 Argon2id 和 AES-256-GCM；禁止明文导出含凭据内容。
