...
A community release line based on the upstream CLIProxyAPI `v6.9.31` codebase.

This repository publishes its own CPA-UV binaries and bundled `management.html` from the same release.

For security, Gemini CLI OAuth and Antigravity OAuth client credentials are no longer hard-coded in this community release. Set `CPA_UV_GEMINI_OAUTH_CLIENT_ID`, `CPA_UV_GEMINI_OAUTH_CLIENT_SECRET`, `CPA_UV_ANTIGRAVITY_OAUTH_CLIENT_ID`, and `CPA_UV_ANTIGRAVITY_OAUTH_CLIENT_SECRET` if you need those login flows.
