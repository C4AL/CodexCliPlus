...
func (e *AntigravityExecutor) refreshToken(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	if err := antigravityAuth.ValidateOAuthCredentials(); err != nil {
		return auth, err
	}
	...
	httpReq, errReq := http.NewRequestWithContext(ctx, http.MethodPost, antigravityAuth.TokenEndpoint, strings.NewReader(form.Encode()))
	if errReq != nil {
		return auth, errReq
	}
