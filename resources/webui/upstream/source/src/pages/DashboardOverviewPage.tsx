...
export function DashboardOverviewPage() {
  ...
  const dt = useCallback(
    (zh: string, fallback?: string) => (isZh ? zh : fallback ?? zh),
    [isZh]
  );
    ...
    return (
      <div className={styles.page}>
        <div className={styles.header}>
          <div>
            <h1 className={styles.pageTitle}>{dt('24 小时概览', '24 小时概览')}</h1>
            <p className={styles.pageSubtitle}>
              {dt('聚合 Codex 账号用量、额度和最近请求。', '聚合 Codex 账号用量、额度和最近请求。')}
            </p>
          </div>
        </div>
        <EmptyState
          title={dt('需要先连接管理接口', '需要先连接管理接口')}
          description={dt(
  ...
  return (
    <div className={styles.page}>
      ...
      <Card title={t('dashboard.account_stats', { defaultValue: dt('账号概览', '账号概览') })}>
        <div className={styles.summaryRow}>
          <div className={styles.summaryPill}>
            ...
            <div>
              <div className={styles.summaryLabel}>{dt('24 小时请求数', '24 小时请求数')}</div>
          ...
          <div className={styles.summaryPill}>
            ...
            <div>
              <div className={styles.summaryLabel}>{dt('24 小时令牌', '24 小时令牌')}</div>
        ...
        {authFilesLoading && accountCards.length === 0 ? (
          <div className={styles.loadingHint}>{dt('正在加载概览数据...', '正在加载概览数据...')}</div>
        ) : accountCards.length === 0 ? (
          <EmptyState
            title={t('dashboard.no_codex_accounts', {
              defaultValue: dt('暂无 Codex 账号', '暂无 Codex 账号'),
            })}
            description={dt(
              ...
              <article key={card.key} className={styles.accountCard}>
                ...
                <div className={styles.accountStats}>
                  <div className={styles.accountStat}>
                    <span className={styles.accountStatLabel}>{dt('请求', '请求')}</span>
                    <span className={styles.accountStatValue}>{card.requests.toLocaleString()}</span>
                  </div>
                  <div className={styles.accountStat}>
                    <span className={styles.accountStatLabel}>{dt('成功率', '成功率')}</span>
                    <span className={styles.accountStatValue}>
                      {card.successRate === null ? '--' : `${card.successRate}%`}
                    </span>
                  </div>
                  <div className={styles.accountStat}>
                    <span className={styles.accountStatLabel}>{dt('令牌', '令牌')}</span>
                    <span className={styles.accountStatValue}>
                ...
                {card.quotaState?.status === 'error' && (
                  <div className={styles.quotaFallback}>
                    {[404, 405, 501].includes(card.quotaState.errorStatus ?? 0)
                      ? dt('本地额度桥不可用', '本地额度桥不可用')
                      : dt('额度数据暂不可用', '额度数据暂不可用')}
                  </div>
                )}
              </article>
            ))}
      ...
      <Card title={dt('最近请求事件', '最近请求事件')}>
        {usageLoading && recentEventRows.length === 0 ? (
          <div className={styles.loadingHint}>{dt('正在加载最近请求...', '正在加载最近请求...')}</div>
        ) : recentEventRows.length === 0 ? (
          <EmptyState
            title={dt('24 小时内暂无请求', '24 小时内暂无请求')}
            description={dt(
