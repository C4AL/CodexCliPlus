...
func (h *Handler) RequestGeminiCLIToken(c *gin.Context) {
	...
	conf, errConf := geminiAuth.NewOAuthConfig(fmt.Sprintf("http://localhost:%d/oauth2callback", geminiAuth.DefaultCallbackPort))
	if errConf != nil {
		c.JSON(http.StatusFailedDependency, gin.H{"error": "gemini_oauth_not_configured", "message": errConf.Error()})
		return
	}
...
func (h *Handler) RequestAntigravityToken(c *gin.Context) {
	...
	if err := antigravity.ValidateOAuthCredentials(); err != nil {
		c.JSON(http.StatusFailedDependency, gin.H{"error": "antigravity_oauth_not_configured", "message": err.Error()})
		return
	}
