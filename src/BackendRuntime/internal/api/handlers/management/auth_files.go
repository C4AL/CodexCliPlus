	...
	if all := c.Query("all"); all == "true" || all == "1" || all == "*" {
		...
		for _, e := range entries {
			...
			if err = os.Remove(full); err == nil {
				...
				if errDisable := h.disableAuth(ctx, full); errDisable != nil {
					c.JSON(500, gin.H{"error": errDisable.Error()})
					return
				}
				deleted++
			}
...
func (h *Handler) writeAuthFile(ctx context.Context, name string, data []byte) error {
	...
	if errWrite := misc.WriteFileAtomically(dst, data, 0o600); errWrite != nil {
...
func (h *Handler) deleteAuthFileByName(ctx context.Context, name string) (string, int, error) {
	...
	if targetID != "" {
		errDisable = h.disableAuth(ctx, targetID)
	} else {
		errDisable = h.disableAuth(ctx, targetPath)
	}
	if errDisable != nil {
		return filepath.Base(name), http.StatusInternalServerError, errDisable
	}
...
func (h *Handler) disableAuth(ctx context.Context, id string) error {
	if h == nil || h.authManager == nil {
		return nil
	}
	...
	go func() {
		...
		waitForFile := func(path string, timeout time.Duration) (map[string]string, error) {
			...
			for {
				...
				if errRead == nil {
					m, errPayload := parseOAuthCallbackPayload(data)
					_ = os.Remove(path)
					if errPayload != nil {
						SetOAuthSessionError(state, "Invalid OAuth callback")
	...
	go func() {
		...
		for {
			...
			if data, errR := os.ReadFile(waitFile); errR == nil {
				...
				if errPayload != nil {
					log.Errorf("Authentication failed: invalid callback payload: %v", errPayload)
					SetOAuthSessionError(state, "Authentication failed: invalid callback")
					return
				}
		...
		email, bodyBytes, errReadUserInfo := readGeminiUserInfoEmail(resp.Body)
		if errReadUserInfo != nil {
			log.Errorf("Failed to read user info response: %v", errReadUserInfo)
			SetOAuthSessionError(state, "Failed to read user info")
			return
		}
		...
		ifToken := buildOAuthTokenMap(nil, token)
	...
	go func() {
		...
		for {
			...
			if data, errR := os.ReadFile(waitFile); errR == nil {
				...
				_ = os.Remove(waitFile)
				if errPayload != nil {
					authErr := codex.NewAuthenticationError(codex.ErrInvalidState, errPayload)
					SetOAuthSessionError(state, "Invalid OAuth callback")
					log.Error(codex.GetUserFriendlyMessage(authErr))
					return
				}
	...
	go func() {
		...
		for {
			...
			if data, errReadFile := os.ReadFile(waitFile); errReadFile == nil {
				payload, errPayload := parseOAuthCallbackPayload(data)
				_ = os.Remove(waitFile)
				if errPayload != nil {
					log.Errorf("Authentication failed: invalid callback payload: %v", errPayload)
					SetOAuthSessionError(state, "Authentication failed: invalid callback")
...
func callGeminiCLI(ctx context.Context, httpClient *http.Client, endpoint string, body any, result any) error {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		bodyText, errRead := readTrimmedResponseBody(resp.Body)
		if errRead != nil {
			return fmt.Errorf("read error response body: %w", errRead)
		}
		return fmt.Errorf("api request failed with status %d: %s", resp.StatusCode, bodyText)
	}

	if result == nil {
		if _, errCopy := io.Copy(io.Discard, resp.Body); errCopy != nil {
			return fmt.Errorf("drain response body: %w", errCopy)
		}
		return nil
	}
...
func fetchGCPProjects(ctx context.Context, httpClient *http.Client) ([]interfaces.GCPProjectProjects, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		bodyText, errRead := readTrimmedResponseBody(resp.Body)
		if errRead != nil {
			return nil, fmt.Errorf("read project list error response body: %w", errRead)
		}
		return nil, fmt.Errorf("project list request failed with status %d: %s", resp.StatusCode, bodyText)
	}
...
func checkCloudAPIIsEnabled(ctx context.Context, httpClient *http.Client, projectID string) (bool, error) {
	...
	for _, service := range requiredServices {
		...
		statusBodyRead := false
		if resp.StatusCode == http.StatusOK {
			bodyBytes, errRead := io.ReadAll(resp.Body)
			if errRead != nil {
				_ = resp.Body.Close()
				return false, fmt.Errorf("read service status response body: %w", errRead)
			}
			statusBodyRead = true
			if gjson.GetBytes(bodyBytes, "state").String() == "ENABLED" {
				if errClose := resp.Body.Close(); errClose != nil {
					return false, fmt.Errorf("close service status response body: %w", errClose)
				}
				continue
			}
		}
		...
		if errRead != nil {
			_ = resp.Body.Close()
			return false, fmt.Errorf("read service enable response body: %w", errRead)
		}
		...
		if resp.StatusCode == http.StatusOK || resp.StatusCode == http.StatusCreated {
			if errClose := resp.Body.Close(); errClose != nil {
				return false, fmt.Errorf("close service enable response body: %w", errClose)
			}
			continue
...
func readGeminiUserInfoEmail(body io.Reader) (string, []byte, error) {
	bodyBytes, errRead := io.ReadAll(body)
	if errRead != nil {
		return "", nil, fmt.Errorf("read user info response body: %w", errRead)
	}
	return gjson.GetBytes(bodyBytes, "email").String(), bodyBytes, nil
}

func parseOAuthCallbackPayload(data []byte) (map[string]string, error) {
	var payload map[string]string
	if errUnmarshal := json.Unmarshal(data, &payload); errUnmarshal != nil {
		return nil, fmt.Errorf("parse OAuth callback payload: %w", errUnmarshal)
	}
	if payload == nil {
...
func readTrimmedResponseBody(body io.Reader) (string, error) {
	bodyBytes, errRead := io.ReadAll(body)
	if errRead != nil {
		return "", errRead
	}
