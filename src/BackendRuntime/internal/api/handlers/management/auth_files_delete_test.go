...
func TestDeleteAuthFile_ReturnsErrorWhenDisableAuthPersistenceFails(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	gin.SetMode(gin.TestMode)

	authDir := t.TempDir()
	fileName := "persist-failure-user.json"
	filePath := filepath.Join(authDir, fileName)
	if errWrite := os.WriteFile(filePath, []byte(`{"type":"codex"}`), 0o600); errWrite != nil {
		t.Fatalf("failed to write auth file: %v", errWrite)
	}
	...
	store.failSaves("disable save failed")
	...
	if deleteRec.Code != http.StatusInternalServerError {
		t.Fatalf("expected delete status %d, got %d with body %s", http.StatusInternalServerError, deleteRec.Code, deleteRec.Body.String())
	}
}
