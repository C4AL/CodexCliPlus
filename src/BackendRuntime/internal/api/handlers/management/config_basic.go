...
func (h *Handler) GetLatestVersion(c *gin.Context) {
	...
	if resp.StatusCode != http.StatusOK {
		message, errRead := latestReleaseUnexpectedStatusMessage(resp.StatusCode, resp.Body)
		if errRead != nil {
			log.WithError(errRead).Debug("failed to read latest version error response body")
		}
		c.JSON(http.StatusBadGateway, gin.H{"error": "unexpected_status", "message": message})
		return
	}
...
func latestReleaseUnexpectedStatusMessage(statusCode int, body io.Reader) (string, error) {
	bodyBytes, errRead := io.ReadAll(io.LimitReader(body, 1024))
	if errRead != nil {
		err := fmt.Errorf("read latest release error response body: %w", errRead)
		return fmt.Sprintf("status %d: %s", statusCode, err.Error()), err
	}
	return fmt.Sprintf("status %d: %s", statusCode, strings.TrimSpace(string(bodyBytes))), nil
}
...
func WriteConfig(path string, data []byte) error {
	...
	return writeConfigFileAtomically(path, data, 0o644, os.Rename)
}

func writeConfigFileAtomically(path string, data []byte, perm os.FileMode, rename func(string, string) error) (err error) {
	tmpFile, err := os.CreateTemp(filepath.Dir(path), ".codexcliplus-config-*.tmp")
	if err != nil {
...
func (h *Handler) PutRequestRetry(c *gin.Context) {
	h.updateNonNegativeIntField(c, func(v int) { h.cfg.RequestRetry = v })
}
...
func (h *Handler) PutMaxRetryInterval(c *gin.Context) {
	h.updateNonNegativeIntField(c, func(v int) { h.cfg.MaxRetryInterval = v })
}
