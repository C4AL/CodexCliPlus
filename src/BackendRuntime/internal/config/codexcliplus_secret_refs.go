...
func (r *codexCliPlusSecretResolver) protectWithSource(path, value, kind, source string) (string, error) {
	...
	reference := saved.URI
	if strings.TrimSpace(reference) == "" && strings.TrimSpace(saved.SecretID) != "" {
		reference = "ccp-secret://" + strings.TrimSpace(saved.SecretID)
	}
	...
	if err := resp.Body.Close(); err != nil {
		return "", fmt.Errorf("secret save response close failed for %s: %w", path, err)
	}
...
func (r *codexCliPlusSecretResolver) resolve(path, value string) (string, error) {
	...
	closeBody = false
	if err := resp.Body.Close(); err != nil {
		return "", fmt.Errorf("secret_ref %s response close failed for %s: %w", secretID, path, err)
	}
