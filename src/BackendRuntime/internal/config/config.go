...
func SaveConfigPreserveComments(configFile string, cfg *Config) error {
	...
	data, err = encodeConfigYAMLNode(&original)
	if err != nil {
		return err
	}
	return writeConfigFileAtomically(configFile, data, os.Rename)
}
...
func SaveConfigPreserveCommentsUpdateNestedScalar(configFile string, path []string, value string) error {
	...
	data, err = encodeConfigYAMLNode(&root)
	if err != nil {
		return err
	}
	return writeConfigFileAtomically(configFile, data, os.Rename)
...
func encodeConfigYAMLNode(node *yaml.Node) ([]byte, error) {
...
func writeConfigFileAtomically(path string, data []byte, rename func(string, string) error) (err error) {
	perm := os.FileMode(0o666)
	if info, errStat := os.Stat(path); errStat == nil {
		perm = info.Mode().Perm()
	} else if !os.IsNotExist(errStat) {
		return fmt.Errorf("stat config file: %w", errStat)
	}
