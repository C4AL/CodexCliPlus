...
func (l *FileRequestLogger) logRequest(url, method string, requestHeaders map[string][]string, body []byte, statusCode int, responseHeaders map[string][]string, response, websocketTimeline, apiRequest, apiResponse, apiWebsocketTimeline []byte, apiResponseErrors []*interfaces.ErrorMessage, force bool, requestID string, requestTimestamp, apiResponseTimestamp time.Time) error {
	...
	logFile, errOpen := os.OpenFile(filePath, os.O_CREATE|os.O_WRONLY|os.O_EXCL, 0644)
	if errOpen != nil {
...
// Format: v1-responses-2025-12-23T195811-42-a1b2c3d4.log
//
// Parameters:
//   - url: The request URL
...
func (l *FileRequestLogger) generateFilename(url string, requestID ...string) string {
	return l.generateFilenameWithTimestamp(url, time.Now().Format("2006-01-02T150405"), requestID...)
}

func (l *FileRequestLogger) generateFilenameWithTimestamp(url, timestamp string, requestID ...string) string {
	...
	sequence := requestLogID.Add(1)

	// Keep the request ID as the final filename component so management lookup by suffix works.
	if len(requestID) > 0 && requestID[0] != "" {
		return fmt.Sprintf("%s-%s-%d-%s.log", sanitized, timestamp, sequence, requestID[0])
	}

	return fmt.Sprintf("%s-%s-%d.log", sanitized, timestamp, sequence)
}
...
func (w *FileStreamingLogWriter) Close() error {
	...
	logFile, errOpen := os.OpenFile(w.logFilePath, os.O_CREATE|os.O_WRONLY|os.O_EXCL, 0644)
	if errOpen != nil {
		w.cleanupTempFiles()
		return fmt.Errorf("failed to create log file: %w", errOpen)
	}
