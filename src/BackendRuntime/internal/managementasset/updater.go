...
func fetchLatestAsset(ctx context.Context, client *http.Client, releaseURL string) (*releaseAsset, string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		body, errRead := io.ReadAll(io.LimitReader(resp.Body, 1024))
		if errRead != nil {
			return nil, "", fmt.Errorf("read release error response body: %w", errRead)
		}
	...
	for i := range release.Assets {
		...
		if strings.EqualFold(asset.Name, managementAssetName) {
			...
			if err = resp.Body.Close(); err != nil {
				return nil, "", fmt.Errorf("close release response body: %w", err)
			}
...
func downloadAsset(ctx context.Context, client *http.Client, downloadURL string) ([]byte, string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		body, errRead := io.ReadAll(io.LimitReader(resp.Body, 1024))
		if errRead != nil {
			return nil, "", fmt.Errorf("read download error response body: %w", errRead)
		}
	...
	closeBody = false
	if err = resp.Body.Close(); err != nil {
		return nil, "", fmt.Errorf("close download response body: %w", err)
	}
