...
func fetchModelsFromRemote(ctx context.Context) (*staticModelsJSON, string) {
	...
	for _, url := range modelsURLs {
		...
		if resp.StatusCode != http.StatusOK {
			closeModelsResponseBody(resp, url)
			cancel()
			log.Debugf("models fetch returned %d from %s", resp.StatusCode, url)
			continue
		}

		data, err := readModelsResponseBody(resp, url)
		cancel()

		if err != nil {
			log.Debugf("models fetch body error from %s: %v", url, err)
			continue
		}
...
func readModelsResponseBody(resp *http.Response, url string) ([]byte, error) {
	data, errRead := io.ReadAll(resp.Body)
	errClose := resp.Body.Close()
	if errRead != nil {
		return nil, fmt.Errorf("read models response body from %s: %w", url, errRead)
	}
	if errClose != nil {
		return nil, fmt.Errorf("close models response body from %s: %w", url, errClose)
	}
	return data, nil
}

func closeModelsResponseBody(resp *http.Response, url string) {
	if errClose := resp.Body.Close(); errClose != nil {
		log.Debugf("models fetch response close error from %s: %v", url, errClose)
	}
}
