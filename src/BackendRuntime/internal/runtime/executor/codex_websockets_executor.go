...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if errDial != nil {
		bodyErr, bodyReadErr := websocketHandshakeBody(respHS)
		if respHS != nil {
		...
		if respHS != nil && respHS.StatusCode > 0 {
			return resp, websocketHandshakeStatusErr(respHS.StatusCode, bodyErr, bodyReadErr)
		}
...
func (e *CodexWebsocketsExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if errDial != nil {
		...
		if respHS != nil && respHS.StatusCode > 0 {
			return nil, websocketHandshakeStatusErr(respHS.StatusCode, bodyErr, bodyReadErr)
		}
		helps.RecordAPIWebsocketError(ctx, e.cfg, "dial", errDial)
		if sess != nil {
			sess.reqMu.Unlock()
...
func websocketHandshakeBody(resp *http.Response) ([]byte, error) {
	...
	closeHTTPResponseBody(resp, "codex websockets executor: close handshake response body error")
	if errRead != nil {
		return nil, fmt.Errorf("read websocket handshake response body: %w", errRead)
	}
	if len(body) == 0 {
		return nil, nil
	}
...
func websocketHandshakeStatusErr(statusCode int, body []byte, bodyReadErr error) statusErr {
	msg := string(body)
	if bodyReadErr != nil {
		msg = bodyReadErr.Error()
	}
