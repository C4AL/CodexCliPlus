...
func TestWebsocketHandshakeBodyReturnsReadFailure(t *testing.T) {
	readErr := errors.New("handshake body failed")
	body := &failingHandshakeReadCloser{err: readErr}

	got, err := websocketHandshakeBody(&http.Response{Body: body})
	if !errors.Is(err, readErr) || !strings.Contains(err.Error(), "read websocket handshake response body") {
		t.Fatalf("websocketHandshakeBody error = %v, want read failure", err)
	}
	if got != nil {
		t.Fatalf("body = %q, want nil", string(got))
	}
	if !body.closed {
...
func TestWebsocketHandshakeStatusErrUsesReadFailureMessage(t *testing.T) {
	readErr := fmt.Errorf("read websocket handshake response body: %w", errors.New("handshake body failed"))

	err := websocketHandshakeStatusErr(http.StatusBadGateway, []byte("upstream body"), readErr)
	if err.code != http.StatusBadGateway {
		t.Fatalf("status code = %d, want %d", err.code, http.StatusBadGateway)
	}
	if !strings.Contains(err.Error(), "read websocket handshake response body") {
		t.Fatalf("error = %q, want read failure message", err.Error())
	}
	if strings.Contains(err.Error(), "upstream body") {
		t.Fatalf("error = %q, want read failure to take precedence", err.Error())
	}
}
...
type failingHandshakeReadCloser struct {
	err    error
	closed bool
}

func (r *failingHandshakeReadCloser) Read([]byte) (int, error) {
	return 0, r.err
}

func (r *failingHandshakeReadCloser) Close() error {
	r.closed = true
	return nil
}
