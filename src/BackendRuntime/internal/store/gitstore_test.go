...
func TestGitTokenStoreSaveMetadataIgnoresStaleFixedTempPath(t *testing.T) {
	root := t.TempDir()
	remoteDir := setupGitRemoteRepository(t, root, "trunk",
		testBranchSpec{name: "trunk", contents: "remote default branch\n"},
	)
...
func TestEnsureEmptyFilePreservesExistingFile(t *testing.T) {
	t.Parallel()

	path := filepath.Join(t.TempDir(), ".gitkeep")
	...
	if string(raw) != "keep me" {
		t.Fatalf("existing file content = %q, want preserved content", raw)
	}
}

func TestEnsureEmptyFileAllowsConcurrentCreate(t *testing.T) {
	t.Parallel()
	...
	for err := range errs {
		if err != nil {
			t.Fatalf("ensureEmptyFile concurrent error: %v", err)
		}
	}
