...
func (h *GeminiCLIAPIHandler) CLIHandler(c *gin.Context) {
	...
	if requestRawURI == "/v1internal:generateContent" {
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			writeGeminiCLIUpstreamErrorResponse(c, resp)
			return
		}

		writeGeminiCLISuccessResponse(c, resp)
	}
}

func writeGeminiCLISuccessResponse(c *gin.Context, resp *http.Response) {
	output, errRead := io.ReadAll(resp.Body)
	errClose := resp.Body.Close()
	if errRead != nil {
		log.Errorf("Failed to read response body: %v", errRead)
		c.JSON(http.StatusBadGateway, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{
				Message: fmt.Sprintf("Failed to read upstream response: %v", errRead),
				Type:    "server_error",
			},
		})
		return
	...
	if errClose != nil {
		...
		c.JSON(http.StatusBadGateway, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{
				Message: fmt.Sprintf("Failed to close upstream response: %v", errClose),
				Type:    "server_error",
			},
		})
		return
...
func writeGeminiCLIUpstreamErrorResponse(c *gin.Context, resp *http.Response) {
	defer func() {
	...
	if errRead != nil {
		log.Errorf("Failed to read upstream error response body: %v", errRead)
		c.JSON(http.StatusBadGateway, handlers.ErrorResponse{
			Error: handlers.ErrorDetail{
				Message: fmt.Sprintf("Failed to read upstream error response: %v", errRead),
				Type:    "server_error",
			},
		})
		return
