	...
	case auth.Storage != nil:
		...
		if raw, errRead := os.ReadFile(path); errRead == nil {
			...
			if !jsonEqual(raw, protectedRaw) {
				if errWrite := writeFileAtomically(path, protectedRaw, 0o600, os.Rename); errWrite != nil {
	...
	case auth.Metadata != nil:
		...
		if existing, errRead := os.ReadFile(path); errRead == nil {
			if !jsonEqual(existing, raw) {
				if errWrite := writeFileAtomically(path, raw, 0o600, os.Rename); errWrite != nil {
					return "", fmt.Errorf("auth filestore: write existing failed: %w", errWrite)
				}
			}
		} else if !os.IsNotExist(errRead) {
			return "", fmt.Errorf("auth filestore: read existing failed: %w", errRead)
		} else if errWrite := writeFileAtomically(path, raw, 0o600, os.Rename); errWrite != nil {
		...
		if projectID == "" {
			...
			if accessToken != "" {
				...
				if errFetch == nil && strings.TrimSpace(fetchedProjectID) != "" {
					...
					if errPersist := persistAuthMetadata(path, metadata); errPersist != nil {
						log.Warnf("auth filestore: failed to persist fetched project_id for %s: %v", path, errPersist)
					}
				}
			}
		}
...
func persistAuthMetadata(path string, metadata map[string]any) error {
	...
	if errMarshal != nil {
		return fmt.Errorf("marshal auth metadata: %w", errMarshal)
	}
	protectedRaw, errProtect := config.ProtectCodexCliPlusAuthJSONForWrite(raw)
	if errProtect != nil {
		return fmt.Errorf("protect auth metadata: %w", errProtect)
	}
	if errWrite := writeFileAtomically(path, protectedRaw, 0o600, os.Rename); errWrite != nil {
		return fmt.Errorf("write auth metadata: %w", errWrite)
	}
	return nil
}

func writeFileAtomically(path string, data []byte, perm fs.FileMode, rename func(string, string) error) (err error) {
	tmpFile, err := os.CreateTemp(filepath.Dir(path), ".codexcliplus-auth-*.tmp")
	if err != nil {
...
func refreshGeminiAccessToken(tokenMap map[string]any, httpClient *http.Client) (string, error) {
	...
	if errRead != nil {
		return "", fmt.Errorf("read refresh response body: %w", errRead)
	}
	...
	if errClose != nil {
		return "", fmt.Errorf("close refresh response body: %w", errClose)
	}
