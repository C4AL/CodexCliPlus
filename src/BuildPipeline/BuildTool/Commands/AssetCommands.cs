...
{
    private const string BackendBuildDate = "2026-05-11T20:16:47Z";
    ...
    public static async Task<int> FetchAssetsAsync(BuildContext context)
    {
        ...
        {
            File.Delete(context.AssetManifestPath);
        }
        ...
        {
            context.Logger.Info("fetching backend and local environment assets in parallel");
            await Task.WhenAll(
                BuildBackendFromRepositorySourceAsync(context, backendTarget),
                LocalEnvironmentAssetCommands.BuildAsync(context)
            );
        }
        else
        {
        ...
        var manifest = await BuildAssetManifest.CreateAsync(
            ...
            context.AssetsRoot,
            excludedRootDirectoryNames: ["webui"],
