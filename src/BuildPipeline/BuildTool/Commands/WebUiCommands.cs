...
public static class WebUiCommands
{
    ...
    public static async Task<int> BuildAsync(BuildContext context)
    {
        return await BuildTiming.TimeAsync(context, "webui", () => BuildCoreAsync(context));
    }

    private static async Task<int> BuildCoreAsync(BuildContext context)
    {
        ...
        context.Logger.Info("preparing WebUI persistent build worktree");
        ...
        {
            context.Logger.Info("using persistent WebUI dependencies");
        }
        else if (Directory.Exists(cachedNodeModulesRoot))
        {
            context.Logger.Info("using cached WebUI dependencies");
        ...
        {
            context.Logger.Info("installing WebUI dependencies in temporary worktree");
        ...
        context.Logger.Info("building WebUI");
        ...
        context.Logger.Info($"WebUI dist generated: {context.WebUiGeneratedDistRoot}");
    ...
    private static string? ValidateBuiltDist(WebUiBundleReport report)
    {
        ...
        {
            return "WebUI build output is missing dist/index.html.";
        }

        if (report.IndexHtmlBytes > MaxIndexHtmlBytes)
        {
            return $"WebUI index.html is too large: {report.IndexHtmlBytes} bytes.";
        }

        if (report.Assets.Count == 0)
        {
            return "WebUI build output is missing dist/assets files.";
        }

        if (report.AssetBytesTotal > MaxAssetBytesTotal)
        {
            return $"WebUI assets are too large: {report.AssetBytesTotal} bytes.";
        }

        return null;
    }

    private static void EnsureLayout(BuildContext context)
    {
        if (!Directory.Exists(context.WebUiSourceRoot))
        {
            throw new DirectoryNotFoundException(
                $"WebUI source directory not found: {context.WebUiSourceRoot}"
            );
        }
        ...
        if (!File.Exists(context.WebUiSourcePackageJsonPath))
        {
            throw new FileNotFoundException(
                $"WebUI package.json not found: {context.WebUiSourcePackageJsonPath}",
        ...
        {
            throw new FileNotFoundException(
                $"WebUI package-lock.json not found: {context.WebUiSourcePackageLockPath}",
        ...
        {
            throw new FileNotFoundException(
                $"WebUI sync metadata not found: {context.WebUiSyncMetadataPath}",
                context.WebUiSyncMetadataPath
            );
        }
    }
