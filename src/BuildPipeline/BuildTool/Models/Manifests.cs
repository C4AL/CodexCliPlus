...
public sealed class BuildAssetManifest
{
    ...
    )
    {
        ...
        foreach (
            var path in Directory
                ...
                .Where(path => !IsExcludedRoot(path, assetsRoot, excludedRoots))
    ...
    )
    {
        ...
        foreach (var manifestPath in GetRequiredAssetManifestPaths())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (
                !Files.Any(file =>
    ...
    private static IEnumerable<string> GetRequiredAssetManifestPaths()
    {
        foreach (var requiredFile in AssetCommands.RequiredFiles)
        {
            yield return $"backend/windows-x64/{requiredFile}";
        }

        yield return "local-environment/manifest.json";
    }
