        ...
        await WriteJsonAsync(
            ...
            new
            {
                ...
                webUi = new
                {
                    ...
                    note = "The desktop shell serves the packaged WebUI from local files instead of a remote URL.",
                },
                installer = new
                {
                    sku = isOnline ? "online" : "offline",
                    precheck = "Setup payload contains dependency-precheck.json so the installer chain can validate WebView2, bundled backend files, and packaged WebUI assets before launch.",
