...
public static class MicaSetupInstallerBuilder
{
    ...
    )
    {
        ...
        )
        {
            var cache = await IncrementalBuildCache.LookupFileAsync(
                context,
                GetUninstallerCacheStage(packageKind),
    ...
    private static string GetUninstallerCacheStage(InstallerPackageKind packageKind)
    {
        return packageKind == InstallerPackageKind.Online
            ? "micasetup-uninstaller-online"
            : "micasetup-uninstaller-offline";
    }
    ...
    private static void CopyLicenseDocuments(BuildContext context, string targetDirectory)
    {
        var documents = new (string Source, string Target)[]
        {
            ...
            (
                Path.Combine(context.RepositoryLicensesRoot, "CliProxyApiManagementCenter.MIT.txt"),
