...
public static class BuildToolApp
{
    ...
    )
    {
        ...
        try
        {
            ...
            logger.Info(
                $"Incremental: {options.Incremental}; Parallel: {options.Parallel}; Compression: {options.Compression.ToArgumentValue()}; Force rebuild: {options.ForceRebuild}"
            );
            ...
            var exitCode = options.Command.ToLowerInvariant() switch
            {
                ...
                "build-webui" => await WebUiCommands.BuildAsync(context),
    ...
    private static void WriteHelp(BuildLogger logger)
    {
        ...
        logger.Info("  --parallel <true|false>          Default: true");
        ...
        for (var index = 1; index < args.Length; index++)
        {
            ...
            switch (arg)
            {
                ...
                case "--incremental":
                    ...
                    break;
                case "--parallel":
                    if (!bool.TryParse(value, out parallel))
                    {
                        error = $"Invalid boolean value for option {arg}: {value}";
                        return false;
                    }
...
)
{
    ...
    public string RepositoryAssetsRoot =>
        Path.Combine(RepositorySourceRoot, "Packaging", "Assets");
    ...
    public string RepositoryLicensePath
    {
        get
        {
            var centralizedLicensePath = Path.Combine(
                RepositoryLicensesRoot,
                "CodexCliPlus.MIT.txt"
            );
            if (File.Exists(centralizedLicensePath))
            {
                return centralizedLicensePath;
            }

            var rootLicensePath = Path.Combine(Options.RepositoryRoot, "LICENSE");
            return File.Exists(rootLicensePath)
                ? rootLicensePath
    ...
    public string MicaSetupRoot =>
        Path.Combine(RepositorySourceRoot, "Packaging", "Installer", "MicaSetup");
    ...
    public string WebUiRoot => Path.Combine(RepositorySourceRoot, "ManagementWeb");
    ...
    public string BackendSourceRoot =>
        Path.Combine(RepositorySourceRoot, "BackendRuntime");
