...
public partial class UsagePage : Page
{
    ...
    private void Render()
    {
        ...
        MetricItems.ItemsSource = new[]
        {
            new ManagementMetricItem(
                snapshot.TotalRequests.ToString(CultureInfo.CurrentCulture),
                "总请求数",
                "全部请求累计"
            ),
            new ManagementMetricItem(
                snapshot.SuccessCount.ToString(CultureInfo.CurrentCulture),
                "成功数",
                "成功返回的请求"
            ),
            new ManagementMetricItem(
                snapshot.FailureCount.ToString(CultureInfo.CurrentCulture),
                "失败数",
                "失败请求累计"
            ),
            new ManagementMetricItem(
                snapshot.TotalTokens.ToString(CultureInfo.CurrentCulture),
                "总 Token",
                "输入、输出、缓存与思考总量"
            ),
        };
        ...
        ProviderItems.ItemsSource = snapshot
            .Apis.Select(pair => new UsageApiSummaryItem(
                pair.Key,
    ...
    private async void ExportButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        File.WriteAllText(
            dialog.FileName,
            JsonSerializer.Serialize(payload.Value, ManagementPageSupport.JsonOptions)
        );
    }
