...
const (
	...
	geminiOAuthClientSecret = "codexcliplus-disabled-oauth-secret"
)
...
const (
	...
	antigravityOAuthClientSecret = "codexcliplus-disabled-oauth-secret"
)
...
func oauthTokenFromMap(values map[string]any) oauth2.Token {
	var token oauth2.Token
	if len(values) == 0 {
		return token
	}
	token.AccessToken = stringValue(values, "access_token")
	token.TokenType = stringValue(values, "token_type")
	token.RefreshToken = stringValue(values, "refresh_token")
	token.ExpiresIn = int64Value(values["expires_in"])
	if expiry := timeValue(values, "expiry"); !expiry.IsZero() {
		token.Expiry = expiry
	}
	return token
}

func timeValue(metadata map[string]any, key string) time.Time {
	if len(metadata) == 0 || key == "" {
		return time.Time{}
	}
...
func buildOAuthTokenMap(base map[string]any, tok *oauth2.Token) map[string]any {
	...
	merged["expiry"] = tok.Expiry.Format(time.RFC3339Nano)
	if tok.ExpiresIn != 0 {
		merged["expires_in"] = float64(tok.ExpiresIn)
	}
	return merged
}
