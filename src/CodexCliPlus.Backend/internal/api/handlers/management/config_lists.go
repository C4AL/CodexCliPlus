...
func decodeListBody[T any](data []byte, out *[]T) bool {
...
func (h *Handler) putStringList(c *gin.Context, set func([]string), after func()) {
	...
	var arr []string
	if !decodeListBody(data, &arr) {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	}
	set(normalizeStringList(arr))
	if after != nil {
		after()
	}
	h.persist(c)
...
func (h *Handler) patchStringList(c *gin.Context, target *[]string, after func()) {
	...
	if body.Index != nil && body.Value != nil && *body.Index >= 0 && *body.Index < len(*target) {
		normalized, ok := normalizeStringListValue(*body.Value)
		if !ok {
			c.JSON(400, gin.H{"error": "value cannot be empty"})
			return
		}
		(*target)[*body.Index] = normalized
		if after != nil {
			after()
		}
		h.persist(c)
	...
	if body.New != nil {
		normalized, ok := normalizeStringListValue(*body.New)
		if !ok {
			c.JSON(400, gin.H{"error": "value cannot be empty"})
			return
		}
		if body.Old != nil {
			...
			for i := range *target {
				if strings.TrimSpace((*target)[i]) != old {
					continue
				}
				(*target)[i] = normalized
				if after != nil {
...
func normalizeStringList(values []string) []string {
	...
	for _, value := range values {
		normalized, ok := normalizeStringListValue(value)
		if !ok {
			continue
		}
...
func normalizeStringListValue(value string) (string, bool) {
...
func (h *Handler) DeleteOpenAICompat(c *gin.Context) {
	...
	if name := strings.TrimSpace(c.Query("name")); name != "" {
		...
		if len(out) == len(h.cfg.OpenAICompatibility) {
			c.JSON(404, gin.H{"error": "item not found"})
			return
		}
...
func (h *Handler) PatchOAuthExcludedModels(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&body); err != nil || body.Provider == nil || body.Models == nil {
		c.JSON(400, gin.H{"error": "invalid body"})
		return
	}
...
func (h *Handler) PutAmpModelMappings(c *gin.Context) {
	...
	h.cfg.AmpCode.ModelMappings = normalizeAmpModelMappings(body.Value)
	h.persist(c)
}
...
func (h *Handler) PatchAmpModelMappings(c *gin.Context) {
	...
	for _, newMapping := range body.Value {
		normalizedMapping, ok := normalizeAmpModelMapping(newMapping)
		if !ok {
			continue
		}
		from := normalizedMapping.From
		if idx, ok := existing[from]; ok {
			h.cfg.AmpCode.ModelMappings[idx] = normalizedMapping
		} else {
			h.cfg.AmpCode.ModelMappings = append(h.cfg.AmpCode.ModelMappings, normalizedMapping)
...
func normalizeAmpModelMappings(mappings []config.AmpModelMapping) []config.AmpModelMapping {
	if len(mappings) == 0 {
		return nil
	}
	out := make([]config.AmpModelMapping, 0, len(mappings))
	for _, mapping := range mappings {
		normalized, ok := normalizeAmpModelMapping(mapping)
		if ok {
			out = append(out, normalized)
		}
	}
...
func normalizeAmpModelMapping(mapping config.AmpModelMapping) (config.AmpModelMapping, bool) {
	mapping.From = strings.TrimSpace(mapping.From)
	mapping.To = strings.TrimSpace(mapping.To)
	if mapping.From == "" || mapping.To == "" {
		return config.AmpModelMapping{}, false
	}
	return mapping, true
}
...
func (h *Handler) DeleteAmpModelMappings(c *gin.Context) {
	...
	if len(newMappings) == len(h.cfg.AmpCode.ModelMappings) {
		c.JSON(404, gin.H{"error": "item not found"})
		return
	}
...
func (h *Handler) DeleteAmpUpstreamAPIKeys(c *gin.Context) {
	...
	if len(newEntries) == len(h.cfg.AmpCode.UpstreamAPIKeys) {
		c.JSON(404, gin.H{"error": "item not found"})
		return
	}
