...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	...
	now := time.Now()
	if err != nil {
		if m.recordRefreshFailure(id, now, err) {
			m.queueRefreshReschedule(id)
		}
		return
	}
	...
	if _, errUpdate := m.Update(ctx, updated); errUpdate != nil {
		log.WithError(errUpdate).Warnf("failed to update refreshed auth %s, %s", auth.Provider, auth.ID)
		if m.recordRefreshFailure(id, now, errUpdate) {
			m.queueRefreshReschedule(id)
		}
	}
}

func (m *Manager) recordRefreshFailure(id string, now time.Time, err error) bool {
	if err == nil {
		return false
	}
	m.mu.Lock()
