...
func TestManagerRefreshAuthRecordsPersistFailure(t *testing.T) {
	ctx := context.Background()
	...
	if got.LastError == nil || !strings.Contains(got.LastError.Message, wantErr.Error()) {
		t.Fatalf("LastError = %#v, want persist failure", got.LastError)
	}
	if got.NextRefreshAfter.IsZero() {
		t.Fatal("NextRefreshAfter is zero, want retry backoff after persist failure")
	}
	if got.LastRefreshedAt.IsZero() {
		t.Fatal("LastRefreshedAt is zero, want successful refresh timestamp retained")
	}
	if got := store.saveCount.Load(); got != 1 {
		t.Fatalf("Save call count = %d, want 1", got)
	}
}
