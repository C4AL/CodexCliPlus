...
func TestManagerRegisterReturnsPersistError(t *testing.T) {
	wantErr := errors.New("save failed")
	store := &countingStore{saveErr: wantErr}
	mgr := NewManager(store, nil, nil)
	auth := &Auth{
		ID:       "auth-1",
		Provider: "antigravity",
	...
	if _, err := mgr.Register(context.Background(), auth); !errors.Is(err, wantErr) {
...
func TestManagerUpdateReturnsPersistError(t *testing.T) {
	wantErr := errors.New("save failed")
	store := &countingStore{saveErr: wantErr}
	mgr := NewManager(store, nil, nil)
	auth := &Auth{
	...
	if _, err := mgr.Update(context.Background(), auth); !errors.Is(err, wantErr) {
