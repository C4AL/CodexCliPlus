...
public sealed class ManagementConfigurationService : IManagementConfigurationService
{
    ...
    public async Task<ManagementApiResponse<ManagementConfigSnapshot>> GetConfigAsync(CancellationToken cancellationToken = default)
    {
        var response = await _apiClient.SendManagementAsync(HttpMethod.Get, "config", cancellationToken: cancellationToken);
        using var document = ManagementJson.Parse(response.Value);
        return ManagementResponseFactory.Map(response, ManagementMappers.MapConfig(document.RootElement));
    }
    ...
    public async Task<ManagementApiResponse<ManagementOperationResult>> PutConfigYamlAsync(string yamlContent, CancellationToken cancellationToken = default)
    {
    ...
    {
        return SendOperationAsync(HttpMethod.Put, path, new Dictionary<string, object?> { ["value"] = value }, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var response = await _apiClient.SendManagementAsync(method, path.TrimStart('/'), body, cancellationToken: cancellationToken);
        using var document = ManagementJson.Parse(response.Value);
        return ManagementResponseFactory.Map(response, ManagementMappers.MapOperation(document.RootElement));
    }
}
