...
public sealed class BuildToolCommandTests : IDisposable
{
    ...
    public async Task FetchAssetsBuildsManagedBackendFromRepositoryOwnedSourceWithoutCloningUpstream()
    {
        ...
        Assert.False(
            ...
            "fetch-assets should build from src/CodexCliPlus.Backend directly."
        );
        Assert.DoesNotContain(
            runner.Calls,
    ...
    [Fact]
    public async Task FetchAssetsUsesTrackedBackendDocsWhenSourceReadmesAreMissing()
    {
        var repositoryRoot = CreateRepositoryWithBackendAssets();
        var sourceRoot = Path.Combine(repositoryRoot, "src", "CodexCliPlus.Backend");
        File.Delete(Path.Combine(sourceRoot, "README.md"));
        File.Delete(Path.Combine(sourceRoot, "README_CN.md"));

        var trackedBackendRoot = Path.Combine(
            repositoryRoot,
            "src", "CodexCliPlus.Backend", "RuntimeAssets", "windows-x64"
        );
        Directory.CreateDirectory(trackedBackendRoot);
        File.WriteAllText(Path.Combine(trackedBackendRoot, "README.md"), "tracked readme");
        File.WriteAllText(Path.Combine(trackedBackendRoot, "README_CN.md"), "tracked readme cn");

        var outputRoot = Path.Combine(_rootDirectory, "out-build-tracked-docs");
        var runner = new BackendSourceBuildProcessRunner();
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        Assert.Equal(
            "tracked readme",
            File.ReadAllText(Path.Combine(backendRoot, "README.md"))
        );
        Assert.Equal(
            "tracked readme cn",
            File.ReadAllText(Path.Combine(backendRoot, "README_CN.md"))
        );
        Assert.Contains(
            "using tracked backend asset fallback",
            output.ToString(),
            StringComparison.Ordinal
        );
    }
    ...
    private string CreateRepositoryWithBackendAssets()
    {
        ...
        BackendSourceBuildProcessRunner.CreatePinnedBackendSource(
            Path.Combine(repositoryRoot, "src", "CodexCliPlus.Backend")
        );
