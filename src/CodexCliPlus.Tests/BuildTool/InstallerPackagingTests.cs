...
public sealed class InstallerPackagingTests : IDisposable
{
    ...
    public async Task PackageInstallerUsesRepoOwnedSourceTemplate()
    {
        ...
        Assert.Contains("window.Closing -= OnMainWindowClosing;", finishViewModel, StringComparison.Ordinal);
        Assert.Contains(
            "private bool cleanupInstallerAfterInstall = true;",
            finishViewModel,
            StringComparison.Ordinal
        );
        Assert.Contains(
    ...
    public void InstallerSelfCleanupTemplatesAvoidShellDelete()
    {
        ...
        var overlayRoot = Path.Combine(
            repositoryRoot,
            "src", "CodexCliPlus.Installer", "MicaSetup",
            "overrides",
            "MicaSetup"
        );
    ...
    private static string LocateRepositoryRoot()
    {
        ...
        while (directory is not null)
        {
            if (
                ...
                && Directory.Exists(Path.Combine(directory.FullName, "src", "CodexCliPlus.Installer", "MicaSetup"))
            )
            {
    ...
    private string CreateRepositoryRoot()
    {
        ...
        Directory.CreateDirectory(Path.Combine(repositoryRoot, "src", "CodexCliPlus.Installer", "MicaSetup"));
        CreateRepoOwnedMicaSourceTemplate(repositoryRoot);
        CreateRepoOwnedMicaSourceTemplates(repositoryRoot);
        var iconRoot = Path.Combine(repositoryRoot, "src", "CodexCliPlus.Assets", "icons");
        ...
        var licenseRoot = Path.Combine(repositoryRoot, "src", "CodexCliPlus.Assets", "licenses");
    ...
    {
        var sourceRoot = Path.Combine(repositoryRoot, "src", "CodexCliPlus.Backend");
    ...
    {
        var sourceRoot = Path.Combine(repositoryRoot, "src", "CodexCliPlus.WebUi");
        ...
        var overlayRoot = Path.Combine(
            repositoryRoot,
            "src", "CodexCliPlus.WebUiOverlay", "CpaUvOverlay"
        );
