...
public sealed class CodexConfigServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task ApplyProfilesAsyncKeepsOriginalConfigWhenReplacementFails()
    {
        ...
        var exception = await Record.ExceptionAsync(() =>
            service.ApplyProfilesAsync(9318, CodexSourceKind.Cpa)
        );
        ...
        Assert.Empty(Directory.EnumerateFiles(_codexHome, "config.toml.*.tmp"));
    }
    ...
    [Fact]
    public async Task SaveCodexUserFileAsyncKeepsOriginalFileWhenReplacementFails()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var configPath = Path.Combine(_codexHome, "config.toml");
        const string originalConfig = "model = \"gpt-5.4\"\n";
        await File.WriteAllTextAsync(configPath, originalConfig);
        var service = new CodexConfigService();
        var snapshot = await service.ReadCodexUserFileAsync("config");
        await using var lockedConfig = new FileStream(
        ...
        Assert.True(exception is IOException or UnauthorizedAccessException, exception?.ToString());
        Assert.Equal(originalConfig, await File.ReadAllTextAsync(configPath));
        var backups = Directory.GetFiles(_codexHome, "config.codexcliplus-source-backup-*.toml");
        Assert.Single(backups);
        Assert.Equal(originalConfig, await File.ReadAllTextAsync(backups[0]));
        Assert.Empty(Directory.EnumerateFiles(_codexHome, "config.toml.*.tmp"));
        Assert.Empty(Directory.EnumerateFiles(_codexHome, ".config.toml.codexcliplus-save-*.tmp"));
    }
    ...
    public void BuildLaunchCommandUsesRequestedProfileAndRepository()
    {
        ...
        var special = service.BuildLaunchCommand(
            CodexSourceKind.Official,
            @"C:\repo with spaces\$(Write-Output unsafe)'s"
        );
    ...
    public async Task InspectAsyncPropagatesCallerCancellation()
    {
        ...
        var exception = await Record.ExceptionAsync(() =>
            service.InspectAsync(null, null, null, "signed-in", cancellation.Token)
        );
