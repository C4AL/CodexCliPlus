...
public sealed class JsonAppConfigurationServicePathModeTests
{
    ...
    [Fact]
    public async Task SaveAsyncKeepsStoredManagementKeyWhenDisablingRememberFails()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-config-remember-failure");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
        ...
        {
            ManagementKey = "remembered-key",
            ManagementKeyReference = "remember-failure-test",
            RememberPassword = true,
            AutoLogin = true,
        };
        await service.SaveAsync(settings);
        var secretPath = Path.Combine(
            pathService.Directories.ConfigDirectory,
            "secrets",
            "remember-failure-test.bin"
        );
        File.SetAttributes(
            pathService.Directories.SettingsFilePath,
            File.GetAttributes(pathService.Directories.SettingsFilePath) | FileAttributes.ReadOnly
        );

        settings.ManagementKey = "session-only-key";
        settings.RememberPassword = false;
        settings.AutoLogin = true;
        var exception = await Record.ExceptionAsync(() => service.SaveAsync(settings));

        File.SetAttributes(pathService.Directories.SettingsFilePath, FileAttributes.Normal);
        ...
        var loadedAfterRestart = await new JsonAppConfigurationService(pathService).LoadAsync();
        Assert.Equal("remembered-key", loadedAfterRestart.ManagementKey);
        Assert.True(loadedAfterRestart.RememberPassword);
        Assert.True(loadedAfterRestart.AutoLogin);
    }

    [Fact]
    public async Task SaveAsyncRestoresStoredManagementKeyWhenRememberedUpdateFails()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-config-remember-rollback");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
        ...
        {
            ManagementKey = "remembered-key",
            ManagementKeyReference = "remember-rollback-test",
            RememberPassword = true,
            AutoLogin = true,
        };
        await service.SaveAsync(settings);
        ...
        settings.ManagementKey = "updated-key";
        var exception = await Record.ExceptionAsync(() => service.SaveAsync(settings));
    ...
    [Fact]
    public async Task SaveAsyncReplacesUnreadableStoredManagementKey()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-config-corrupt-secret");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
        ...
        var secretPath = Path.Combine(
            pathService.Directories.ConfigDirectory,
            "secrets",
            "corrupt-remember-test.bin"
        );
        Directory.CreateDirectory(Path.GetDirectoryName(secretPath)!);
        await File.WriteAllBytesAsync(secretPath, [1, 2, 3, 4]);
        ...
        await service.SaveAsync(
            new AppSettings
            {
                ...
                ManagementKeyReference = "corrupt-remember-test",
                RememberPassword = true,
                AutoLogin = true,
            }
        );
        ...
        var loadedAfterRestart = await new JsonAppConfigurationService(pathService).LoadAsync();
        Assert.Equal("replacement-key", loadedAfterRestart.ManagementKey);
        Assert.True(loadedAfterRestart.RememberPassword);
        Assert.True(loadedAfterRestart.AutoLogin);
    }
    ...
    [Fact]
    public async Task LoadAsyncReturnsPlaintextManagementKeyWhenMigrationWriteFails()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-config-plaintext-readonly");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
        ...
        await File.WriteAllTextAsync(
            ...
            {
              "backendPort": 8317,
              "managementKey": "legacy-plaintext-key"
            }
            """
        );
        File.SetAttributes(
        ...
        try
        {
            settings = await new JsonAppConfigurationService(pathService).LoadAsync();
        }
        finally
        {
            File.SetAttributes(pathService.Directories.SettingsFilePath, FileAttributes.Normal);
        ...
        var persistedJson = await File.ReadAllTextAsync(pathService.Directories.SettingsFilePath);
        Assert.Equal("legacy-plaintext-key", settings.ManagementKey);
        Assert.True(settings.RememberPassword);
        Assert.True(settings.AutoLogin);
        Assert.Contains("\"managementKey\": \"legacy-plaintext-key\"", persistedJson);
        Assert.False(
            File.Exists(
                Path.Combine(
                    pathService.Directories.ConfigDirectory,
                    "secrets",
                    $"{AppConstants.DefaultManagementKeyReference}.bin"
                )
            )
        );
    }
    ...
    [Fact]
    public async Task LoadAsyncReturnsLegacyRememberSettingsWhenMigrationWriteFails()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-config-legacy-readonly");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
        ...
        Assert.True(settings.RememberPassword);
        Assert.True(settings.AutoLogin);
        Assert.Contains("\"rememberManagementKey\": true", persistedJson);
        Assert.DoesNotContain("rememberPassword", persistedJson, StringComparison.Ordinal);
    }
