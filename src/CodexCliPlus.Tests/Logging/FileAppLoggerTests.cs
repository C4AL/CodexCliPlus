...
public sealed class FileAppLoggerTests : IDisposable
{
    ...
    [Fact]
    public void LoggingMethodsDoNotThrowWhenLogFileIsLocked()
    {
        var pathService = new TestPathService(_rootDirectory);
        var logger = new FileAppLogger(pathService);

        File.WriteAllText(logger.LogFilePath, "existing");
        ...
        Assert.Null(Record.Exception(() => logger.Info("locked log info")));
        Assert.Null(Record.Exception(() => logger.Warn("locked log warning")));
        Assert.Null(Record.Exception(() => logger.LogError("locked log error")));
    }
