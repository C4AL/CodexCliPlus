...
public sealed class ManagementUsageServiceTests
{
    ...
    [Fact]
    public async Task GetUsageAsyncSkipsMalformedRequestDetails()
    {
        var client = new RecordingManagementApiClient(
            CreateResponse(
                """
                {"usage":{"apis":{"codex":{"models":{"gpt-5":{"details":[null,"bad",{"timestamp":"2026-04-23T01:02:03Z","source":"codex-cli","auth_index":"1","failed":false,"tokens":{"input_tokens":1,"output_tokens":2,"total_tokens":3}}]}}}}}}
                """
            )
        );
        var service = new ManagementUsageService(client);
        ...
        Assert.Equal(HttpMethod.Get, client.LastMethod);
        Assert.Equal("usage", client.LastPath);
        var api = Assert.Single(response.Value.Apis);
        var model = Assert.Single(api.Value.Models);
        var detail = Assert.Single(model.Value.Details);
        Assert.Equal("codex-cli", detail.Source);
        Assert.Equal("1", detail.AuthIndex);
        Assert.False(detail.Failed);
        Assert.Equal(3, detail.Tokens.TotalTokens);
    }
