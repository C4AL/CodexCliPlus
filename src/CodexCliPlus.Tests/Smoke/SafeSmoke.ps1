...
function Invoke-StaticVerification {
    ...
    $syncMetadataPath = Join-Path $RepositoryRoot "src\CodexCliPlus.WebUi\sync.json"
    if (-not (Test-Path -LiteralPath $syncMetadataPath)) {
        Add-Finding -List $failures -Kind "Vendored WebUI metadata missing" -Detail "src\CodexCliPlus.WebUi\sync.json was not found."
    }
    else {
        ...
        foreach ($requiredToken in @(
            ...
            if ($syncMetadata -notmatch [Regex]::Escape($requiredToken)) {
                Add-Finding -List $failures -Kind "Vendored WebUI metadata mismatch" -Detail ("src\CodexCliPlus.WebUi\sync.json is missing '{0}'." -f $requiredToken)
            }
        }
    }

    $webUiBridgePath = Join-Path $RepositoryRoot "src\CodexCliPlus.WebUi\src\desktop\bridge.ts"
    if (-not (Test-Path -LiteralPath $webUiBridgePath)) {
        Add-Finding -List $failures -Kind "Vendored WebUI desktop bridge missing" -Detail "src\CodexCliPlus.WebUi\src\desktop\bridge.ts was not found."
    }

    $embeddedGitPath = Join-Path $RepositoryRoot "src\CodexCliPlus.WebUi\.git"
    if (Test-Path -LiteralPath $embeddedGitPath) {
        Add-Finding -List $failures -Kind "Embedded upstream repository metadata present" -Detail "src\CodexCliPlus.WebUi\.git should not be shipped as a nested repository."
    }
    ...
    $legacyNamedFiles = $activeFiles | Where-Object {
        $_.FullName -notlike "*\src\CodexCliPlus.Tests\Smoke\*" -and (
    ...
    $bundledReadmes = @(
        (Join-Path $RepositoryRoot "src\CodexCliPlus.Backend\RuntimeAssets\windows-x64\README.md"),
        (Join-Path $RepositoryRoot "src\CodexCliPlus.Backend\RuntimeAssets\windows-x64\README_CN.md")
