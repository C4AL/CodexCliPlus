...
let codexRouteStateInFlight: Promise<CodexRouteState> | null = null;
let codexUserFilesInFlight: Promise<CodexUserFileSnapshot[]> | null = null;
...
function normalizeCodexUserFileResponse(
  message: Record<string, unknown>
): CodexUserFileBridgeResponse {
...
function registerCodexUserFileRequest(requestId: string): Promise<CodexUserFileBridgeResponse> {
...
export function requestCodexRouteState(): Promise<CodexRouteState> {
  if (codexRouteStateInFlight) {
    return codexRouteStateInFlight;
  }
  ...
  const inFlight = request.finally(() => {
    if (codexRouteStateInFlight === inFlight) {
      codexRouteStateInFlight = null;
    }
  });
  codexRouteStateInFlight = inFlight;
  try {
...
export function requestCodexUserFiles(): Promise<CodexUserFileSnapshot[]> {
  if (codexUserFilesInFlight) {
    return codexUserFilesInFlight;
  }
  ...
  const inFlight = request.finally(() => {
    if (codexUserFilesInFlight === inFlight) {
      codexUserFilesInFlight = null;
    }
  });
  codexUserFilesInFlight = inFlight;
  try {
