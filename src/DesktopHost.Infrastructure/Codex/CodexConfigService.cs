...
{
    private const string ManagedRootStart = "# BEGIN CPAD MANAGED ROOT";
    private const string ManagedRootEnd = "# END CPAD MANAGED ROOT";
    private const string ManagedTablesStart = "# BEGIN CPAD MANAGED TABLES";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!Toml.TryToModel<TomlTable>(mergedContent, out _, out DiagnosticsBag diagnostics))
        {
            throw new InvalidOperationException(
                $"写入 Codex 配置前语法校验失败：{string.Join("; ", diagnostics.Select(item => item.ToString()))}");
        }
        ...
        CancellationToken cancellationToken)
    {
        var backupPath = Path.Combine(
            configDirectory,
            $"config.cpad-backup-{DateTimeOffset.Now:yyyyMMddHHmmss}.toml");
        await File.WriteAllTextAsync(
            backupPath,
            existingContent,
            new UTF8Encoding(encoderShouldEmitUTF8Identifier: false),
