...
{
    private readonly Func<CancellationToken, Task<(int ExitCode, string StandardOutput, string StandardError)>> _whereRunner;
    private readonly Func<string> _npmFallbackPathProvider;

    public CodexLocator()
        : this(
            cancellationToken => ProcessCapture.RunAsync("where.exe", "codex", cancellationToken: cancellationToken),
            () => Path.Combine(
    ...
    public CodexLocator(
        Func<CancellationToken, Task<(int ExitCode, string StandardOutput, string StandardError)>> whereRunner,
