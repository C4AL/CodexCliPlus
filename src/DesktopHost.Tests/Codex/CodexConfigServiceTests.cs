...
namespace DesktopHost.Tests.Codex;

[Collection("CodexConfigService")]
public sealed class CodexConfigServiceTests : IDisposable
{
    private readonly string _codexHome = Path.Combine(Path.GetTempPath(), $"cpad-codex-home-{Guid.NewGuid():N}");
    ...
    public async Task ApplyProfilesAsyncPreservesExistingConfigAndCreatesBackup()
    {
        ...
        var backups = Directory.GetFiles(_codexHome, "config.cpad-backup-*.toml");
    ...
    public async Task ApplyProfilesAsyncThrowsForInvalidTomlAndKeepsOriginalFile()
    {
        ...
        Assert.Contains("语法校验失败", exception.Message, StringComparison.Ordinal);
