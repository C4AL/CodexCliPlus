...
namespace DesktopHost.Tests.Platform;

public sealed class DirectoryAccessServiceTests : IDisposable
{
    private readonly string _rootDirectory = Path.Combine(Path.GetTempPath(), $"cpad-dir-test-{Guid.NewGuid():N}");
    ...
    [Fact]
    public void GetWriteAccessErrorReturnsChineseMessageForInvalidDirectoryTarget()
    {
        var service = CreateService();
        ...
        Assert.StartsWith("目录不可写：", error, StringComparison.Ordinal);
    }
