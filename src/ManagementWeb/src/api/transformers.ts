...
import { buildHeaderObject } from '@/lib/headers';
...
export const normalizeConfigResponse = (raw: unknown): Config => {
  ...
  const redisUsageQueueRetentionSeconds =
    raw['redis-usage-queue-retention-seconds'] ?? raw.redisUsageQueueRetentionSeconds;
  if (
    typeof redisUsageQueueRetentionSeconds === 'number' &&
    Number.isFinite(redisUsageQueueRetentionSeconds)
  ) {
    config.redisUsageQueueRetentionSeconds = redisUsageQueueRetentionSeconds;
  } else if (
    typeof redisUsageQueueRetentionSeconds === 'string' &&
    redisUsageQueueRetentionSeconds.trim() !== ''
  ) {
    const parsed = Number(redisUsageQueueRetentionSeconds);
    if (Number.isFinite(parsed)) {
      config.redisUsageQueueRetentionSeconds = parsed;
    }
  }
