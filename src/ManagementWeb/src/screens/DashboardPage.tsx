...
import { useAuthStore, useConfigStore, useModelsStore, useNotificationStore } from '@/state';
import { apiKeysApi, providersApi, authFilesApi } from '@/api';
...
export function DashboardPage() {
  ...
  const renderLocalDependencyItem = (
    item: LocalDependencyItem,
    variant: 'ready' | 'issue' = 'issue'
  ) => {
    ...
    return (
      <div
        key={item.id}
        className={`${styles.localDependencyItem} ${
          variant === 'ready' ? styles.localDependencyItemReady : styles.localDependencyItemIssue
        }`}
      >
                  ...
                  <div className={styles.localReadyGroup}>
                    ...
                    {localReadyItemsOpen && (
                      <div className={styles.localReadyGroupItems}>
                        {localReadyItems.map((item) => renderLocalDependencyItem(item, 'ready'))}
                      </div>
                    )}
                  </div>
                )}
                {localIssueItems.length > 0 && (
                  <div className={styles.localIssueItems}>
                    {localIssueItems.map((item) => renderLocalDependencyItem(item, 'issue'))}
                  </div>
                )}
              </div>
            )}
