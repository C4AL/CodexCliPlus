...
import type { LocalDependencySnapshot } from '@/api/desktopBridge';
...
vi.mock('@/api/desktopBridge', async () => {
  const actual = await vi.importActual<typeof import('@/api/desktopBridge')>('@/api/desktopBridge');
  return {
    ...actual,
...
describe('DashboardPage local environment loading', () => {
  ...
  it('places ready local environment items first but keeps them collapsed by default', async () => {
    ...
    const readyRow = getDependencyRow('PowerShell');
    expect(readyRow).toBeInTheDocument();
    expect(readyRow.className).toContain('localDependencyItemReady');
    expect(readyRow.closest('[class*="localReadyGroupItems"]')).toBeInTheDocument();
    expect(codexRow.className).toContain('localDependencyItemIssue');
    expect(codexRow.closest('[class*="localIssueItems"]')).toBeInTheDocument();
  });
