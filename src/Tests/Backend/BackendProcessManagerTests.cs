...
public sealed class BackendProcessManagerTests : IDisposable
{
    ...
    public async Task StopAsyncFastExitIsIdempotentWhenAlreadyStopped()
    {
        ...
        Assert.False(IsStopRequested(manager));
    }

    [Fact]
    public async Task StartAsyncExternalCancellationPropagatesAfterIdempotentStop()
    {
        using var cancellation = new CancellationTokenSource();
        var manager = CreateManager(onEnsureCreated: token =>
        {
            cancellation.Cancel();
            token.ThrowIfCancellationRequested();
        });
        await manager.StopAsync(BackendProcessStopOptions.FastExit);

        var exception = await Record.ExceptionAsync(async () =>
            await manager.StartAsync(cancellation.Token)
        );

        Assert.IsAssignableFrom<OperationCanceledException>(exception);
        Assert.Equal(BackendStateKind.Stopped, manager.CurrentStatus.State);
        Assert.False(IsStopRequested(manager));
        Assert.False(IsActiveOperationCancellationRequested(manager));
    }
    ...
    [Fact]
    public async Task StopAsyncFastExitContinuesRuntimeCleanupWhenFileIsLocked()
    {
        var manager = CreateManager();
        var managedProcess = new TestManagedProcess();
        SetManagedProcess(manager, managedProcess);
        var runtimeDirectory = Path.Combine(_rootDirectory, "runtime");
        var lockedPath = Path.Combine(runtimeDirectory, "locked.txt");
        var staleDirectory = Path.Combine(runtimeDirectory, "stale-dir");
        Directory.CreateDirectory(staleDirectory);
        await File.WriteAllTextAsync(lockedPath, "locked");
        await File.WriteAllTextAsync(Path.Combine(staleDirectory, "stale.txt"), "stale");
    ...
    [Fact]
    public async Task StopAsyncAlreadyCancelledLeavesManagerStateUntouched()
    {
        var manager = CreateManager();
        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();
        ...
        var exception = await Record.ExceptionAsync(async () =>
            await manager.StopAsync(BackendProcessStopOptions.FastExit, cancellation.Token)
        );

        Assert.IsAssignableFrom<OperationCanceledException>(exception);
        Assert.False(IsStopRequested(manager));
        Assert.Equal(BackendProcessStopOptions.Default, GetRequestedStopOptions(manager));
        Assert.False(IsActiveOperationCancellationRequested(manager));
        Assert.Null(GetManagedProcess(manager));
    }

    [Fact]
    public async Task StopAsyncCancellationKeepsRunningProcessForRetry()
    {
        var manager = CreateManager();
        var managedProcess = new TestManagedProcess();
        using var cancellation = new CancellationTokenSource();
        managedProcess.BeforeStopCompletes = cancellation.Cancel;
        SetManagedProcess(manager, managedProcess);
        ...
        Assert.IsAssignableFrom<OperationCanceledException>(exception);
        Assert.False(managedProcess.HasExited);
        Assert.False(managedProcess.Disposed);
        Assert.Same(managedProcess, GetManagedProcess(manager));
        Assert.False(IsStopRequested(manager));

        managedProcess.BeforeStopCompletes = null;

        var stopped = await manager.StopAsync(BackendProcessStopOptions.FastExit);

        Assert.Equal(BackendStateKind.Stopped, stopped.State);
        Assert.Equal(2, managedProcess.StopCallCount);
        Assert.True(managedProcess.HasExited);
        Assert.True(managedProcess.Disposed);
        Assert.Null(GetManagedProcess(manager));
    }

    [Fact]
    public async Task StopAsyncCancellationDoesNotSuppressUnexpectedExit()
    {
        var manager = CreateManager();
        var managedProcess = new TestManagedProcess();
        using var cancellation = new CancellationTokenSource();
        ...
        managedProcess.RaiseExited();

        Assert.Equal(BackendStateKind.Error, manager.CurrentStatus.State);
        Assert.Contains("unexpectedly", manager.CurrentStatus.LastError, StringComparison.Ordinal);
    }

    [Fact]
    public async Task StartAsyncExternalCancellationPropagatesAndClearsStartingState()
    {
        using var cancellation = new CancellationTokenSource();
        var manager = CreateManager(onEnsureCreated: token =>
        {
    ...
    [Fact]
    public async Task StopAsyncCancellationWhileStartupIsStoppingDoesNotPoisonNextOperation()
    {
        using var startupEntered = new ManualResetEventSlim();
        using var startupCancellationObserved = new ManualResetEventSlim();
        ...
        var manager = CreateManager(onEnsureCreated: token =>
        {
            ...
            startupCancellationObserved.Set();
            Assert.True(releaseStartup.Wait(TimeSpan.FromSeconds(5), CancellationToken.None));
            token.ThrowIfCancellationRequested();
        });

        var startTask = Task.Run(() => manager.StartAsync());
        Assert.True(startupEntered.Wait(TimeSpan.FromSeconds(5)));

        using var stopCancellation = new CancellationTokenSource();
        var stopTask = manager.StopAsync(BackendProcessStopOptions.FastExit, stopCancellation.Token);

        try
        {
            Assert.True(startupCancellationObserved.Wait(TimeSpan.FromSeconds(5)));
            stopCancellation.Cancel();

            var stopException = await Record.ExceptionAsync(async () => await stopTask);

            Assert.IsAssignableFrom<OperationCanceledException>(stopException);
        }
        finally
        {
            releaseStartup.Set();
        ...
        Assert.Equal(BackendStateKind.Stopped, startResult.State);
        Assert.False(IsActiveOperationCancellationRequested(manager));
        Assert.False(IsStopRequested(manager));
        Assert.Equal(BackendProcessStopOptions.Default, GetRequestedStopOptions(manager));
    ...
    private BackendProcessManager CreateManager(Action<CancellationToken>? onEnsureCreated = null)
    {
        var pathService = new TestPathService(_rootDirectory, onEnsureCreated);
    ...
    private static IManagedProcess? GetManagedProcess(BackendProcessManager manager)
    {
        var field = typeof(BackendProcessManager).GetField(
            "_managedProcess",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic
        ...
        Assert.NotNull(field);
        return (IManagedProcess?)field.GetValue(manager);
    }

    private static bool IsStopRequested(BackendProcessManager manager)
    {
        var field = typeof(BackendProcessManager).GetField(
            "_stopRequested",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic
        ...
        Assert.NotNull(field);
        return Assert.IsType<bool>(field.GetValue(manager));
    }

    private static BackendProcessStopOptions GetRequestedStopOptions(
        BackendProcessManager manager
    )
    {
        var field = typeof(BackendProcessManager).GetField(
            "_requestedStopOptions",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic
        );
        Assert.NotNull(field);
        return Assert.IsType<BackendProcessStopOptions>(field.GetValue(manager));
    }

    private static bool IsActiveOperationCancellationRequested(BackendProcessManager manager)
    {
        var field = typeof(BackendProcessManager).GetField(
            "_operationCts",
            System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic
        ...
        Assert.NotNull(field);
        var cancellation = Assert.IsType<CancellationTokenSource>(field.GetValue(manager));
    ...
    private sealed class TestManagedProcess : IManagedProcess
    {
        ...
        public Action? BeforeStopCompletes { get; set; }
    ...
    {
        private readonly Action<CancellationToken>? _onEnsureCreated;

        public TestPathService(
            string rootDirectory,
            Action<CancellationToken>? onEnsureCreated = null
        )
        {
            _onEnsureCreated = onEnsureCreated;
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            _onEnsureCreated?.Invoke(cancellationToken);
            cancellationToken.ThrowIfCancellationRequested();
