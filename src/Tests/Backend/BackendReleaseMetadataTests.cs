...
{
    [Fact]
    public void BackendReleaseMetadataPinsCliProxyApi703WindowsAsset()
    {
        Assert.Equal("7.0.3", BackendReleaseMetadata.Version);
        Assert.Equal("v7.0.3", BackendReleaseMetadata.ReleaseTag);
        Assert.Equal("CLIProxyAPI_7.0.3_windows_amd64.zip", BackendReleaseMetadata.AssetName);
        Assert.Equal(
        ...
        Assert.Equal(
            "bd8c05a830a36b2e5181bb5c8596a2c8d85c3dbc",
    ...
    public void BackendWindowsManifestMatchesPinnedRuntimeMetadata()
    {
        ...
        var manifestPath = Path.Combine(
            repositoryRoot,
            "src", "BackendRuntime", "RuntimeAssets", "windows-x64",
    ...
    public void BackendSourceManifestDescribesRepositoryOwnedSourceBuild()
    {
        ...
        Assert.Equal("src/BackendRuntime", root.GetProperty("sourceRoot").GetString());
    ...
    {
        Assert.Equal("7.0.3", InvokeParseBackendVersion("CLIProxyAPI Version: 7.0.3"));
        Assert.Equal(
            "7.0.3",
            InvokeParseBackendVersion("CLIProxyAPI Version: v7.0.3\r\nBuild Date: 2026-05-11")
        );
        ...
        Assert.True(InvokeIsExpectedBackendVersion("7.0.3"));
        Assert.True(InvokeIsExpectedBackendVersion("v7.0.3"));
