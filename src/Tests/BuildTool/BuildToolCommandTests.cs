...
public sealed class BuildToolCommandTests : IDisposable
{
    ...
    public async Task HelpListsBuildToolCommands()
    {
        ...
        Assert.Contains("--parallel", output.ToString(), StringComparison.Ordinal);
    ...
    [Fact]
    public void FileMaterializerRejectsMaterializeFileOntoItself()
    {
        Directory.CreateDirectory(_rootDirectory);
        var sourcePath = Path.Combine(_rootDirectory, "materialize-file.txt");
        File.WriteAllText(sourcePath, "content");

        var exception = Assert.Throws<InvalidOperationException>(() =>
            FileMaterializer.MaterializeFile(sourcePath, sourcePath)
        );
    ...
    [Fact]
    public void FileMaterializerRejectsMaterializeDirectoryIntoSourceDescendant()
    {
        var sourceRoot = Path.Combine(_rootDirectory, "materialize-source");
        var targetRoot = Path.Combine(sourceRoot, "nested-target");
        Directory.CreateDirectory(sourceRoot);
        File.WriteAllText(Path.Combine(sourceRoot, "file.txt"), "content");

        var exception = Assert.Throws<InvalidOperationException>(() =>
            FileMaterializer.MaterializeDirectory(sourceRoot, targetRoot)
        );
    ...
    [Fact]
    public void FileMaterializerRejectsMaterializeDirectoryIntoDotDotPrefixedSourceDescendant()
    {
        var sourceRoot = Path.Combine(_rootDirectory, "materialize-source-dotdot");
        var targetRoot = Path.Combine(sourceRoot, "..nested-target");
        Directory.CreateDirectory(sourceRoot);
        File.WriteAllText(Path.Combine(sourceRoot, "file.txt"), "content");
    ...
    [Fact]
    public void SafeFileSystemRejectsCopyIntoSourceDescendant()
    {
        var sourceRoot = Path.Combine(_rootDirectory, "copy-source");
        var targetRoot = Path.Combine(sourceRoot, "nested-target");
        Directory.CreateDirectory(sourceRoot);
        File.WriteAllText(Path.Combine(sourceRoot, "marker.txt"), "keep");

        var exception = Assert.Throws<InvalidOperationException>(() =>
            SafeFileSystem.CopyDirectory(sourceRoot, targetRoot)
        );
    ...
    [Fact]
    public void SafeFileSystemRejectsCopyIntoDotDotPrefixedSourceDescendant()
    {
        var sourceRoot = Path.Combine(_rootDirectory, "copy-source-dotdot");
        var targetRoot = Path.Combine(sourceRoot, "..nested-target");
        Directory.CreateDirectory(sourceRoot);
        File.WriteAllText(Path.Combine(sourceRoot, "marker.txt"), "keep");
    ...
    [Fact]
    public async Task VerifyAsyncReportsAssetHashMismatch()
    {
        var assetsRoot = Path.Combine(_rootDirectory, "assets-verify");
        var backendRoot = Path.Combine(assetsRoot, "backend", "windows-x64");
        ...
        {
            var path = Path.Combine(backendRoot, requiredFile);
            var content = $"original {requiredFile}";
            await File.WriteAllTextAsync(path, content);
            files.Add(
                new BuildAssetFile
                {
                    Path = $"backend/windows-x64/{requiredFile}",
                    Size = Encoding.UTF8.GetByteCount(content),
                    Sha256 = ComputeSha256Hex(content),
                }
            );
        }
        var localEnvironmentRoot = Path.Combine(assetsRoot, "local-environment");
        Directory.CreateDirectory(localEnvironmentRoot);
        await File.WriteAllTextAsync(
            Path.Combine(localEnvironmentRoot, "manifest.json"),
            "{}"
        );
        files.Add(
            new BuildAssetFile
            {
                Path = "local-environment/manifest.json",
                Size = Encoding.UTF8.GetByteCount("{}"),
                Sha256 = ComputeSha256Hex("{}"),
            }
        );

        var managedOriginalContent = $"original {BackendExecutableNames.ManagedExecutableFileName}";
        await File.WriteAllTextAsync(
            Path.Combine(backendRoot, BackendExecutableNames.ManagedExecutableFileName),
            new string('x', managedOriginalContent.Length)
        );
        var manifest = new BuildAssetManifest { Files = files };

        var failures = await manifest.VerifyAsync(assetsRoot);

        Assert.Contains(
            $"Asset hash mismatch: backend/windows-x64/{BackendExecutableNames.ManagedExecutableFileName}",
            failures
        );
        Assert.DoesNotContain(
            failures,
            failure => failure.StartsWith("Required asset missing", StringComparison.Ordinal)
        );
    }
    ...
    public async Task FetchAssetsBuildsManagedBackendFromRepositoryOwnedSourceWithoutCloningUpstream()
    {
        ...
        Assert.True(File.Exists(Path.Combine(backendRoot, "config.example.yaml")));
        Assert.False(File.Exists(Path.Combine(backendRoot, "LICENSE")));
        Assert.False(File.Exists(Path.Combine(backendRoot, "README.md")));
        Assert.False(File.Exists(Path.Combine(backendRoot, "README_CN.md")));
        ...
        Assert.False(
            ...
            "fetch-assets should build from src/BackendRuntime directly."
        );
        Assert.DoesNotContain(
            runner.Calls,
        ...
        Assert.Contains("main.Version=7.0.3", goBuildCall.Arguments[ldflagsIndex]);
    ...
    [Fact]
    public async Task FetchAssetsDoesNotCopyBackendReadmesIntoRuntimeAssets()
    {
        var repositoryRoot = CreateRepositoryWithBackendAssets();
        ...
        var outputRoot = Path.Combine(_rootDirectory, "out-build-runtime-files");
        var runner = new BackendSourceBuildProcessRunner();
        using var output = new StringWriter();
        using var error = new StringWriter();
    ...
    {
        var sourceRoot = Path.Combine(FindRepositoryRoot(), "src", "BuildPipeline", "BuildTool");
    ...
    [Fact]
    public async Task BuildWebUiUsesIntegratedSourceWorktree()
    {
        var repositoryRoot = CreateRepositoryWithIntegratedWebUiSource();
        var outputRoot = Path.Combine(_rootDirectory, "out");
        using var output = new StringWriter();
        using var error = new StringWriter();
        var logger = new BuildLogger(output, error);
        var runner = new WebUiRecordingProcessRunner();
        ...
        var exitCode = await WebUiCommands.BuildAsync(context);
    ...
    [Fact]
    public async Task BuildWebUiReusesCachedNodeModulesForSamePackageLock()
    {
        var repositoryRoot = CreateRepositoryWithIntegratedWebUiSource();
        ...
        Assert.Equal(0, await WebUiCommands.BuildAsync(context));
        Assert.Equal(0, await WebUiCommands.BuildAsync(context));
    ...
    [Fact]
    public async Task BuildWebUiForceRebuildKeepsPersistentNodeModules()
    {
        var repositoryRoot = CreateRepositoryWithIntegratedWebUiSource();
        ...
        Assert.Contains(
            "using persistent WebUI dependencies",
            output.ToString(),
            StringComparison.Ordinal
        );
        Assert.Equal(string.Empty, error.ToString());
    ...
    public async Task ProcessRunnerKillsStartedProcessWhenCancelled()
    {
        ...
        var pidPath = Path.Combine(
            _rootDirectory,
            $"buildtool-process-{Guid.NewGuid():N}.pid"
        );
    ...
    [Fact]
    public void RequirePublishRootRejectsMissingPackagedWebUiAssets()
    {
    ...
    [Fact]
    public void RequirePublishRootRejectsMissingLocalEnvironmentPackage()
    {
        var publishRoot = Path.Combine(_rootDirectory, "publish-local-env");
        Directory.CreateDirectory(publishRoot);
        File.WriteAllText(Path.Combine(publishRoot, "CodexCliPlus.exe"), "codexcliplus");
        Directory.CreateDirectory(Path.Combine(publishRoot, "assets", "backend", "windows-x64"));
        CreateStubExecutable(
        ...
        Assert.Contains(
            Path.Combine("assets", "local-environment", "manifest.json"),
            exception.FileName ?? exception.Message,
            StringComparison.OrdinalIgnoreCase
        );
    }
    ...
    private string CreateRepositoryWithBackendAssets()
    {
        ...
        File.WriteAllText(Path.Combine(repositoryRoot, "LICENSE"), "root license");
        var licenseRoot = Path.Combine(
            repositoryRoot,
            "src", "Packaging", "Assets",
            "licenses"
        );
        ...
        {
            File.WriteAllText(Path.Combine(licenseRoot, fileName), fileName);
        }

        BackendSourceBuildProcessRunner.CreatePinnedBackendSource(
            Path.Combine(repositoryRoot, "src", "BackendRuntime")
        );
    ...
    private string CreateRepositoryWithIntegratedWebUiSource()
    {
    ...
    private static void CreateUpdatePackage(string packageRoot)
    {
        ...
        CreateZipWithExecutableEntries(
            ...
            new Dictionary<string, byte[]>
            {
                ...
                ["payload/assets/local-environment/manifest.json"] = Encoding.UTF8.GetBytes("{}"),
            }
        );
    }
    ...
    private sealed class WebUiRecordingProcessRunner : IProcessRunner
    {
