...
public sealed class InstallerPackagingTests : IDisposable
{
    ...
    public async Task PackageOfflineInstallerBuildsExecutableAndKeepsFullPayloadWhenRequested()
    {
        ...
        Assert.False(installerPlan.CleanupInstallerAfterInstallDefault);
    ...
    [Fact]
    public async Task PackageOfflineInstallerTrimsMicaSetupBuildCacheByDefault()
    {
        var repositoryRoot = CreateRepositoryRoot();
        var outputRoot = Path.Combine(_rootDirectory, "out-trimmed-cache");
        CreatePublishRoot(outputRoot);
        CreateWebView2Cache(outputRoot);
        var payloadCacheRoot = Path.Combine(outputRoot, "cache", "incremental", "payload");
        Directory.CreateDirectory(payloadCacheRoot);
        var oldPayloadCachePath = Path.Combine(
            payloadCacheRoot,
            "offline-8.8.8-win-x64-optimal.7z"
        );
        File.WriteAllBytes(oldPayloadCachePath, [1, 2, 3]);
        ...
        Assert.True(Directory.Exists(distRoot));
        Assert.True(File.Exists(Path.Combine(distRoot, "Program.cs")));
        Assert.False(Directory.Exists(Path.Combine(distRoot, "bin")));
        Assert.False(Directory.Exists(Path.Combine(distRoot, "obj")));
        Assert.True(File.Exists(Path.Combine(distRoot, "Resources", "Setups", "Uninst.exe")));
        ...
        Assert.False(Directory.Exists(Path.Combine(stageRoot, "output")));
        Assert.True(
            File.Exists(Path.Combine(payloadCacheRoot, "offline-9.9.9-win-x64-optimal.7z"))
        );
        Assert.False(File.Exists(oldPayloadCachePath));
        Assert.Contains(
            "kept trimmed MicaSetup source cache",
            output.ToString(),
            StringComparison.Ordinal
        );
        Assert.Contains(
            "removed old payload archive cache",
            output.ToString(),
            StringComparison.Ordinal
        );
    }
    ...
    public async Task PackageInstallerUsesRepoOwnedSourceTemplate()
    {
        ...
        var installerSelfCleanupHelper = File.ReadAllText(
            Path.Combine(distRoot, "Helper", "Setup", "InstallerSelfCleanupHelper.cs")
        );
        ...
        Assert.DoesNotContain(
            "InstallerSelfCleanupHelper.TryRunSelfCleanupMode()",
            setupProgram,
            StringComparison.Ordinal
        );
        ...
        Assert.DoesNotContain("LoadLicenseInfo", setupMainViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("LicenseRead", setupMainViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("ShowOrHideLincenseInfo", setupMainViewModel, StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            "IsChecked=\"{Binding CleanupInstallerAfterInstall, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}\"",
            finishPage,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains("window.Closing -= OnMainWindowClosing;", finishViewModel, StringComparison.Ordinal);
        Assert.Contains(
            "private bool cleanupInstallerAfterInstall;",
            finishViewModel,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "set => SetProperty(ref cleanupInstallerAfterInstall, value);",
            finishViewModel,
            StringComparison.Ordinal
        );
        Assert.Contains("private bool installerDeleteScheduled;", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("UpdateFocusedElementBindings();", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("Keyboard.FocusedElement", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("BindingExpressionBase", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("bindingExpression.UpdateSource();", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("if (!CleanupInstallerAfterInstall || installerDeleteScheduled)", finishViewModel, StringComparison.Ordinal);
        Assert.Contains("installerDeleteScheduled = true;", finishViewModel, StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            "current path is the installed application; skip delete.",
            finishViewModel,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain("InstallerDeleteRetryCount", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("InstallerDeleteEnvironmentVariable", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("CODEXCLIPLUS_INSTALLER_TO_DELETE", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("ScheduleDelayedInstallerDelete", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("BuildInstallerDeleteCommand", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("GetCommandProcessorPath", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("QuoteCmdPath", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("QuoteCmdCommand", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("\"cmd.exe\"", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("/d /s /c", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("del /f /q", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("for /l %i in (1,1,", finishViewModel, StringComparison.Ordinal);
        Assert.DoesNotContain("timeout /t 1 /nobreak", finishViewModel, StringComparison.Ordinal);
        Assert.Contains(
            "private const string InstallerCleanupArgument = \"codexcliplus-installer-cleanup\";",
            installerSelfCleanupHelper,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains("ScheduleDelete(string installerPath, string cleanupHelperPath)", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("NormalizeCleanupHelperPath(cleanupHelperPath, targetPath)", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("FileName = helperPath", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("--\" + InstallerCleanupArgument", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("UseShellExecute = false", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("CreateNoWindow = true", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.Contains("ProcessWindowStyle.Hidden", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("File.Copy", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("CreateHelperCopy", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("ComputeSha256", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("IsSameFileContent", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("Kernel32.MoveFileEx", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("cmd.exe", installerSelfCleanupHelper, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("powershell.exe", installerSelfCleanupHelper, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("del /f /q", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("Remove-Item", installerSelfCleanupHelper, StringComparison.Ordinal);
        Assert.DoesNotContain("Start-Sleep", installerSelfCleanupHelper, StringComparison.Ordinal);
    ...
    [Fact]
    public void InstallerSelfCleanupTemplatesAvoidShellDelete()
    {
        var repositoryRoot = LocateRepositoryRoot();
        var overlayRoot = Path.Combine(
            repositoryRoot,
            "src", "Packaging", "Installer", "MicaSetup",
            "overrides",
            "MicaSetup"
        );
        var programTemplate = File.ReadAllText(Path.Combine(overlayRoot, "Program.cs.template"));
        var finishViewModelTemplate = File.ReadAllText(
            Path.Combine(overlayRoot, "ViewModels", "Inst", "FinishViewModel.cs.template")
        );
        var selfCleanupHelperTemplate = File.ReadAllText(
            Path.Combine(
                overlayRoot,
                "Helper",
                "Setup",
                "InstallerSelfCleanupHelper.cs.template"
            )
        );
        ...
        Assert.Contains(
            "InstallerSelfCleanupHelper.ScheduleDelete(currentFullPath, installedExecutablePath);",
            finishViewModelTemplate,
            StringComparison.Ordinal
        );
        Assert.Contains("FileName = helperPath", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.Contains("--\" + InstallerCleanupArgument", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("File.Copy", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("File.Delete(targetPath);", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("Kernel32.MoveFileEx", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("IsSameFileContent", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("CODEXCLIPLUS_INSTALLER_TO_DELETE", finishViewModelTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("cmd.exe", finishViewModelTemplate, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("powershell.exe", finishViewModelTemplate, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("cmd.exe", selfCleanupHelperTemplate, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("powershell.exe", selfCleanupHelperTemplate, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("del /f /q", selfCleanupHelperTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("Remove-Item", selfCleanupHelperTemplate, StringComparison.Ordinal);
    }

    [Fact]
    public void InstallerMainPageTemplateOmitsLicenseAgreementAcceptance()
    {
        var repositoryRoot = LocateRepositoryRoot();
        var mainPageTemplate = File.ReadAllText(
            Path.Combine(
                repositoryRoot,
                "src", "Packaging", "Installer", "MicaSetup",
                "source-template",
                "Views", "Inst", "MainPage.xaml"
            )
        );

        Assert.DoesNotContain("HaveReadAndApproved", mainPageTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("UserLicenseAgreement", mainPageTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("LicenseRead", mainPageTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("ShowOrHideLincenseInfoCommand", mainPageTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("LicenseShown", mainPageTemplate, StringComparison.Ordinal);
        Assert.DoesNotContain("IsEnabled=\"{Binding CanStart}\"", mainPageTemplate, StringComparison.Ordinal);
    }

    [Fact]
    public void InstallerMaintenanceModeTemplatesOfferUpdateRepairAndUninstall()
    {
        var repositoryRoot = LocateRepositoryRoot();
        var micaRoot = Path.Combine(
            repositoryRoot,
            "src", "Packaging", "Installer", "MicaSetup"
        );
        var sourceRoot = Path.Combine(micaRoot, "source-template");
        var overlayRoot = Path.Combine(micaRoot, "overrides", "MicaSetup");

        var optionSource = File.ReadAllText(Path.Combine(sourceRoot, "Option.cs"));
        var uninstallRegistryHelper = File.ReadAllText(
            Path.Combine(sourceRoot, "Helper", "System", "RegistyUninstallHelper.cs")
        );
        var mainWindowSource = File.ReadAllText(
            Path.Combine(sourceRoot, "Views", "MainWindow.xaml.cs")
        );
        var mainPageTemplate = File.ReadAllText(
            Path.Combine(sourceRoot, "Views", "Inst", "MainPage.xaml")
        );
        var setupMainViewModelTemplate = File.ReadAllText(
        ...
        var finishPageTemplate = File.ReadAllText(
            Path.Combine(overlayRoot, "Views", "Inst", "FinishPage.xaml.template")
        );

        Assert.Contains("public SetupMaintenanceAction MaintenanceAction", optionSource, StringComparison.Ordinal);
        Assert.Contains("Update,", optionSource, StringComparison.Ordinal);
        Assert.Contains("Repair,", optionSource, StringComparison.Ordinal);
        Assert.Contains("Uninstall,", optionSource, StringComparison.Ordinal);
        Assert.Contains("info.DisplayVersion = subKey?.GetValue(\"DisplayVersion\") as string;", uninstallRegistryHelper, StringComparison.Ordinal);
        Assert.Contains("Option.Current.MaintenanceAction == SetupMaintenanceAction.Uninstall", mainWindowSource, StringComparison.Ordinal);

        Assert.Contains("Visibility=\"{Binding IsInstalled, Converter={StaticResource BoolToVisibilityInvertedConverter}}\"", mainPageTemplate, StringComparison.Ordinal);
        Assert.Contains("Visibility=\"{Binding IsInstalled, Converter={StaticResource BoolToVisibilityConverter}}\"", mainPageTemplate, StringComparison.Ordinal);
        Assert.Contains("Command=\"{Binding StartUpdateCommand}\"", mainPageTemplate, StringComparison.Ordinal);
        Assert.Contains("Command=\"{Binding StartRepairCommand}\"", mainPageTemplate, StringComparison.Ordinal);
        Assert.Contains("Command=\"{Binding StartUninstallCommand}\"", mainPageTemplate, StringComparison.Ordinal);
        Assert.Contains("Content=\"{Binding RepairButtonText}\"", mainPageTemplate, StringComparison.Ordinal);

        Assert.Contains("RegistyUninstallHelper.Read(Option.Current.KeyName)", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("public bool IsInstalled", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("StartMaintenance(SetupMaintenanceAction.Update)", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("StartMaintenance(SetupMaintenanceAction.Repair)", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("StartMaintenance(SetupMaintenanceAction.Uninstall)", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("正在更新 CodexCliPlus", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("正在修复 CodexCliPlus", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("正在卸载 CodexCliPlus", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("Option.Current.KeepMyData = true;", setupMainViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("Option.Current.KeepMyData = KeepMyData;", setupMainViewModelTemplate, StringComparison.Ordinal);

        Assert.Contains("RunMaintenanceUninstallAsync", installViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("RunMaintenanceRepairCleanup", installViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("RunUninstall(0d, 35d)", installViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("SetupMaintenanceAction.Uninstall", installViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("InstallProgress = installProgressBase + (progress * installProgressRange);", installViewModelTemplate, StringComparison.Ordinal);

        Assert.Contains("CanOpenInstalledApplication", finishViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("CanCleanupInstallerAfterInstall", finishViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains("if (!CanCleanupInstallerAfterInstall)", finishViewModelTemplate, StringComparison.Ordinal);
        Assert.Contains(
            "Visibility=\"{Binding CanOpenInstalledApplication, Converter={StaticResource BoolToVisibilityConverter}}\"",
            finishPageTemplate,
            StringComparison.Ordinal
        );
    }
    ...
    public async Task PackageOfflineInstallerReusesIncrementalCacheAndInvalidatesOnPublishChange()
    {
        ...
        Assert.Contains(
            "micasetup-uninstaller-offline cache hit",
            output.ToString(),
            StringComparison.Ordinal
        );
        Assert.Equal(string.Empty, error.ToString());
    ...
    private static string LocateRepositoryRoot()
    {
        ...
        {
            if (
                File.Exists(Path.Combine(directory.FullName, "CodexCliPlus.sln"))
                && Directory.Exists(Path.Combine(directory.FullName, "src", "Packaging", "Installer", "MicaSetup"))
            )
            {
    ...
    private string CreateRepositoryRoot()
    {
        ...
        Directory.CreateDirectory(Path.Combine(repositoryRoot, "src", "Packaging", "Installer", "MicaSetup"));
        CreateRepoOwnedMicaSourceTemplate(repositoryRoot);
        CreateRepoOwnedMicaSourceTemplates(repositoryRoot);
        var iconRoot = Path.Combine(repositoryRoot, "src", "Packaging", "Assets", "icons");
        ...
        var licenseRoot = Path.Combine(repositoryRoot, "src", "Packaging", "Assets", "licenses");
        Directory.CreateDirectory(licenseRoot);
        foreach (
            var fileName in new[]
            {
                "CodexCliPlus.MIT.txt",
                "CLIProxyAPI.MIT.txt",
        ...
        {
            File.WriteAllText(
                Path.Combine(licenseRoot, fileName),
                fileName.Equals("NOTICE.txt", StringComparison.Ordinal) ? "notice" : fileName
            );
        }
    ...
    private string CreateReleaseRepositoryRoot()
    {
        ...
        CreateIntegratedWebUiSource(repositoryRoot);
    ...
    {
        var sourceRoot = Path.Combine(repositoryRoot, "src", "BackendRuntime");
    ...
    private static void CreateIntegratedWebUiSource(string repositoryRoot)
    {
        var sourceRoot = Path.Combine(repositoryRoot, "src", "ManagementWeb");
        ...
        File.WriteAllText(Path.Combine(sourceRoot, "src", "message.txt"), "integrated");
        File.WriteAllText(Path.Combine(sourceRoot, "sync.json"), "{}");
    ...
    private static void CreateRepoOwnedMicaSourceTemplate(string repositoryRoot)
    {
        ...
        Directory.CreateDirectory(Path.Combine(sourceTemplateRoot, "Views", "Inst"));
        ...
        File.WriteAllText(
            Path.Combine(sourceTemplateRoot, "Views", "Inst", "MainPage.xaml"),
            "<Page />"
        );
    }
    ...
    private static void CreateRepoOwnedMicaSourceTemplates(string repositoryRoot)
    {
        ...
        File.WriteAllText(
            Path.Combine(overlayRoot, "Views", "Inst", "FinishPage.xaml.template"),
            """
            <CheckBox Content="完成后删除安装包" IsChecked="{Binding CleanupInstallerAfterInstall, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" />
            """
        );
        File.WriteAllText(
            Path.Combine(overlayRoot, "ViewModels", "Inst", "FinishViewModel.cs.template"),
            ...
            public sealed class FinishViewModel
            {
                ...
                public bool CleanupInstallerAfterInstall
                {
                    get => cleanupInstallerAfterInstall;
                    set => SetProperty(ref cleanupInstallerAfterInstall, value);
                }
                ...
                private void CleanupOriginalInstallerAfterInstall()
                {
                    ...
                    string currentFullPath = "setup.exe";
                    _ = "current path is the installed application; skip delete.";
                    string installedExecutablePath = "CodexCliPlus.exe";
                    InstallerSelfCleanupHelper.ScheduleDelete(currentFullPath, installedExecutablePath);
                }
                ...
                private static void UpdateFocusedElementBindings()
                {
                    ...
                    LocalValueEnumerator localValues = focusedElement.GetLocalValueEnumerator();
                    while (localValues.MoveNext())
                    {
                        if (localValues.Current.Value is BindingExpressionBase bindingExpression)
                        {
        ...
        File.WriteAllText(
            Path.Combine(overlayRoot, "Helper", "Setup", "InstallerSelfCleanupHelper.cs.template"),
            """
            public static class InstallerSelfCleanupHelper
            {
                ...
                {
                    string targetPath = installerPath;
                    string helperPath = NormalizeCleanupHelperPath(cleanupHelperPath, targetPath);
                    int parentProcessId = 0;
                    string arguments = "--" + InstallerCleanupArgument + " --" + TargetArgument + "=" + targetPath;
