...
public sealed class DependencyHealthServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task EvaluateAsyncDeletesCredentialProbeWhenCancellationFollowsSave()
    {
        var rootDirectory = CreateRootDirectory();
        var pathService = new TestPathService(rootDirectory);
        SeedHealthyManagedState(pathService);
        using var cancellation = new CancellationTokenSource();
        var credentialStore = new CancellingAfterSaveCredentialStore(cancellation);
        ...
        var exception = await Record.ExceptionAsync(async () =>
            await service.EvaluateAsync(new BackendStatusSnapshot(), cancellation.Token)
        );

        Assert.IsAssignableFrom<OperationCanceledException>(exception);
        Assert.Equal(0, credentialStore.SecretCount);
    }

    [Fact]
    public async Task EvaluateAsyncPropagatesPreCanceledTokenBeforeSynchronousChecks()
    {
        var rootDirectory = CreateRootDirectory();
        var pathService = new TestPathService(rootDirectory);
        using var cancellation = new CancellationTokenSource();
        ...
        var service = new DependencyHealthService(
            ...
            () => throw new InvalidOperationException("测试应在同步检查前停止。"),
            () => throw new InvalidOperationException("测试应在后端资产检查前停止。")
        );
    ...
    [Fact]
    public async Task EvaluateAsyncPropagatesCancellationFromUpdateProbe()
    {
        var rootDirectory = CreateRootDirectory();
        var pathService = new TestPathService(rootDirectory);
        SeedHealthyManagedState(pathService);
        ...
        var service = CreateService(
            pathService,
            new InMemoryCredentialStore(),
            new CancellingUpdateCheckService(cancellation)
        );
    ...
    public async Task EvaluateAsyncReturnsResourcePackIssueWhenManagedResourceFileIsMissing()
    {
        ...
        var pathService = new TestPathService(rootDirectory);
        SeedHealthyManagedState(pathService);
        File.Delete(Path.Combine(pathService.Directories.BackendDirectory, "config.example.yaml"));
    ...
    private sealed class CancellingAfterSaveCredentialStore : ISecureCredentialStore
    {
        private readonly CancellationTokenSource _cancellation;
        private readonly Dictionary<string, string> _secrets = new(StringComparer.Ordinal);

        public CancellingAfterSaveCredentialStore(CancellationTokenSource cancellation)
        {
    ...
    private sealed class CancellingUpdateCheckService : IUpdateCheckService
    {
        private readonly CancellationTokenSource _cancellation;

        public CancellingUpdateCheckService(CancellationTokenSource cancellation)
        {
            _cancellation = cancellation;
        }
        ...
        )
        {
            _cancellation.Cancel();
            cancellationToken.ThrowIfCancellationRequested();
            throw new InvalidOperationException("测试应在取消后停止。");
        }
    }
}
