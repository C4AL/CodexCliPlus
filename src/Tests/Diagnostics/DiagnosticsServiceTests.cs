...
public sealed class DiagnosticsServiceTests : IDisposable
{
    ...
    [Fact]
    public void CreateErrorSnapshotCreatesUniqueFilesForRapidRequests()
    {
        var pathService = new TestPathService(_rootDirectory);
        var service = new DiagnosticsService(pathService, new TestBuildInfo());

        var firstPath = service.CreateErrorSnapshot(
            "First failure",
            null,
            null,
        ...
        var secondPath = service.CreateErrorSnapshot(
            "Second failure",
            null,
            null,
            new BackendStatusSnapshot(),
        ...
        Assert.Contains("First failure", File.ReadAllText(firstPath), StringComparison.Ordinal);
        Assert.Contains("Second failure", File.ReadAllText(secondPath), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void ExportPackageCreatesUniqueArchivesForRapidRequests()
    {
        var pathService = new TestPathService(_rootDirectory);
        var service = new DiagnosticsService(pathService, new TestBuildInfo());

        var firstPath = service.ExportPackage(
            new BackendStatusSnapshot { Message = "first" },
            new CodexStatusSnapshot(),
            new DependencyCheckResult()
        );
        var secondPath = service.ExportPackage(
            new BackendStatusSnapshot { Message = "second" },
            new CodexStatusSnapshot(),
            new DependencyCheckResult()
        );
        ...
        using var firstArchive = ZipFile.OpenRead(firstPath);
        using var secondArchive = ZipFile.OpenRead(secondPath);
        Assert.Contains(
            "Backend message: first",
            ReadEntryText(firstArchive, "report.txt"),
            StringComparison.Ordinal
        );
        Assert.Contains(
            "Backend message: second",
            ReadEntryText(secondArchive, "report.txt"),
            StringComparison.Ordinal
        );
    }

    [Fact]
    public async Task ExportPackageSkipsLockedOptionalFiles()
    {
        var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        Directory.CreateDirectory(pathService.Directories.LogsDirectory);
        var logPath = Path.Combine(pathService.Directories.LogsDirectory, "desktop.log");
        await File.WriteAllTextAsync(logPath, "authorization: Bearer locked-token");
        ...
        Assert.True(File.Exists(packagePath));
        using var archive = ZipFile.OpenRead(packagePath);
        Assert.NotNull(archive.GetEntry("report.txt"));
        Assert.Null(archive.GetEntry("desktop.log"));
    }
