...
public sealed class LocalDependencyHealthServiceTests
{
    ...
    public async Task CheckAsyncReturnsRequiredEnvironmentLatestCodexRepairCapability()
    {
        ...
        Assert.Equal("一键修复", capability.Name);
    ...
    public async Task CheckAsyncOffersWingetRepairWhenWingetIsMissing()
    {
        ...
        Assert.True(capability.IsAvailable);
        var nodeCapability = Assert.Single(
            snapshot.RepairCapabilities,
            item => item.ActionId == LocalDependencyRepairActionIds.InstallNodeNpm
        );
        Assert.True(nodeCapability.IsAvailable);
        Assert.Contains("内置流程", nodeCapability.Detail, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task CheckAsyncRunsNpmVersionAndPrefixProbesConcurrently()
    {
        var fixture = LocalEnvironmentFixture.CreateHealthy();
        fixture.ProcessRunner.DelayOn(
            "cmd.exe",
    ...
    [Fact]
    public async Task CheckAsyncRunsPowerShellPathProbesConcurrently()
    {
        var fixture = LocalEnvironmentFixture.CreateHealthy();
        fixture.ProcessRunner.DelayOn(
            "where.exe",
        ...
        stopwatch.Stop();
        var powershell = Assert.Single(snapshot.Items, item => item.Id == "powershell");
        Assert.Equal(LocalDependencyStatus.Ready, powershell.Status);
        Assert.True(stopwatch.Elapsed < TimeSpan.FromMilliseconds(900));
    }

    [Fact]
    public async Task CheckAsyncAllowsColdPowerShellVersionProbeWithinSnapshotBudget()
    {
        var fixture = LocalEnvironmentFixture.CreateHealthy();
        fixture.ProcessRunner.DelayOn(
        ...
        var powershell = Assert.Single(snapshot.Items, item => item.Id == "powershell");
        Assert.Equal(LocalDependencyStatus.Ready, powershell.Status);
        Assert.Equal("7.5.0", powershell.Version);
        Assert.True(stopwatch.Elapsed < TimeSpan.FromSeconds(2));
    }
