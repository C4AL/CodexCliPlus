...
public sealed class OfflineBuilderTests : IDisposable
{
    ...
    public void OptionsUseExpectedDefaultsAndParsePassThroughValues()
    {
        ...
        Assert.True(custom.CleanArtifacts);
        Assert.False(custom.Parallel);
    ...
    public async Task ToolchainResolverReusesUsableToolCache()
    {
        ...
        Assert.Contains(
            Path.Combine(repositoryRoot, "artifacts", "toolcache", "go", "1.26.3"),
    ...
    public async Task ToolchainResolverDownloadsMissingToolsIntoLocalCache()
    {
        ...
        try
        {
            ...
            Assert.Contains(
                downloader.Downloads,
                uri => uri.Contains("go1.26.3.windows-amd64.zip", StringComparison.Ordinal)
            );
    ...
    [Fact]
    public async Task BuildServiceRestoresBuildsAndCopiesInstallerToDesktop()
    {
        var repositoryRoot = CreateRepositoryRoot();
        ...
        Assert.Contains("build-release", runner.Calls[1].Arguments);
        Assert.Contains("--packages", runner.Calls[1].Arguments);
        Assert.Contains("offline", runner.Calls[1].Arguments);
        Assert.Contains("--compression", runner.Calls[1].Arguments);
        Assert.Contains("optimal", runner.Calls[1].Arguments);
        Assert.Contains("--force-rebuild", runner.Calls[1].Arguments);
        Assert.Contains("installer", runner.Calls[1].Arguments);
        Assert.Contains("--parallel", runner.Calls[1].Arguments);
        Assert.Contains("true", runner.Calls[1].Arguments);
        Assert.DoesNotContain(runner.Calls, call => call.Arguments.Contains("clean-artifacts"));
    }

    [Fact]
    public async Task BuildServiceCanRunLegacyArtifactCleanupWhenRequested()
    {
        var repositoryRoot = CreateRepositoryRoot();
        var outputRoot = Path.Combine(_rootDirectory, "out-clean");
        var desktopRoot = Path.Combine(_rootDirectory, "desktop-clean");
        Directory.CreateDirectory(desktopRoot);
        ...
        Assert.Contains("clean-artifacts", runner.Calls[3].Arguments);
        Assert.Contains("--parallel", runner.Calls[2].Arguments);
        Assert.Contains("false", runner.Calls[2].Arguments);
    }
    ...
    private string CreateRepositoryRoot()
    {
        ...
        Directory.CreateDirectory(Path.Combine(repositoryRoot, "src", "BuildPipeline", "BuildTool"));
        File.WriteAllText(Path.Combine(repositoryRoot, "CodexCliPlus.sln"), string.Empty);
        File.WriteAllText(
            Path.Combine(
                repositoryRoot,
                "src", "BuildPipeline", "BuildTool",
