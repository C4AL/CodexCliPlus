...
public sealed class AppPathServiceTests
{
    ...
    [Fact]
    public void PersistenceProbeDoesNotDeleteExistingFixedProbeFile()
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("codexcliplus-path-probe");
        var persistenceDirectory = Path.Combine(overrideRoot, "persistence");
        Directory.CreateDirectory(persistenceDirectory);
        var fixedProbePath = Path.Combine(persistenceDirectory, ".write-test");
        File.WriteAllText(fixedProbePath, "existing");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);

        var service = new AppPathService();

        Assert.Equal(Path.GetFullPath(persistenceDirectory), service.Directories.PersistenceDirectory);
        Assert.True(File.Exists(fixedProbePath));
        Assert.Equal("existing", File.ReadAllText(fixedProbePath));
    }
