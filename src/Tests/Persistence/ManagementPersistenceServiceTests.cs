...
public sealed class ManagementPersistenceServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task SyncUsageSnapshotIgnoresLockedExpiredBackupCleanupFailure()
    {
        using var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        var oldBackupDirectory = Path.Combine(
        ...
        var lockedBackupPath = Path.Combine(oldBackupDirectory, "locked.json");
        await File.WriteAllTextAsync(lockedBackupPath, "{}");
        await using var lockedBackup = new FileStream(
            lockedBackupPath,
            FileMode.Open,
            FileAccess.Read,
            FileShare.None
        ...
        var exception = await Record.ExceptionAsync(() => service.SyncUsageSnapshotAsync());

        Assert.Null(exception);
        Assert.Equal(1, service.GetStatus().UsageEventCount);
        Assert.Null(service.GetStatus().LastPersistenceError);
        if (OperatingSystem.IsWindows())
        {
            Assert.True(Directory.Exists(oldBackupDirectory));
        }
    }
    ...
    [Fact]
    public async Task SyncUsageSnapshotCancellationDoesNotPersistLastError()
    {
        using var pathService = new TestPathService(_rootDirectory);
        using var cancellation = new CancellationTokenSource();
        var usageService = new FakeUsageService
        {
            BeforeExportUsage = token =>
            {
                cancellation.Cancel();
        ...
        var exception = await Record.ExceptionAsync(async () =>
            await service.SyncUsageSnapshotAsync(cancellation.Token)
        );
    ...
    [Fact]
    public async Task MigrateLegacySnapshotAsyncPropagatesCancellation()
    {
        using var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        ...
        await File.WriteAllTextAsync(legacySnapshotPath, "{}", Encoding.UTF8);
        var store = new UsageKeeperStore(pathService.Directories.PersistenceDirectory);
        using var cancellation = new CancellationTokenSource();
        cancellation.Cancel();

        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            store.MigrateLegacySnapshotAsync(cancellation.Token)
        );

        Assert.True(File.Exists(legacySnapshotPath));
    }

    [Fact]
    public async Task MigrateLegacySnapshotAsyncIsolatesInvalidLegacyJson()
    {
        using var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        var legacySnapshotPath = Path.Combine(
        ...
        await File.WriteAllTextAsync(legacySnapshotPath, "{ not-json", Encoding.UTF8);
        var store = new UsageKeeperStore(pathService.Directories.PersistenceDirectory);

        var migrated = await store.MigrateLegacySnapshotAsync(CancellationToken.None);
        ...
        Assert.Single(
            Directory.EnumerateFiles(
                pathService.Directories.PersistenceDirectory,
                "usage-snapshot.json.invalid.*"
            )
        );

        var secondMigration = await store.MigrateLegacySnapshotAsync(CancellationToken.None);
    ...
    [Fact]
    public async Task MigrateLegacySnapshotAsyncCompletesWhenLegacyFileDeleteIsLocked()
    {
        using var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        var legacySnapshotPath = Path.Combine(
        ...
        await File.WriteAllTextAsync(
            legacySnapshotPath,
            UsageKeeperJson.SerializeExport(
                CreatePayload(
                    "2026-04-29T00:00:00Z",
                    CreateDetail("2026-04-28T23:00:00Z", "codex-a", "1", totalTokens: 30)
                )
            ),
            Encoding.UTF8
        );
        ...
        var exception = await Record.ExceptionAsync(async () =>
            migrated = await store.MigrateLegacySnapshotAsync(CancellationToken.None)
        );

        Assert.Null(exception);
        Assert.True(migrated);
        Assert.Equal(1, (await store.GetStatusAsync(CancellationToken.None)).EventCount);
        if (OperatingSystem.IsWindows())
        {
            Assert.True(File.Exists(legacySnapshotPath));
        }
    ...
    [Fact]
    public async Task SyncLogsSnapshotKeepsExistingSnapshotWhenReplacementFails()
    {
        if (!OperatingSystem.IsWindows())
        {
            return;
        ...
        var logsSnapshotPath = Path.Combine(keeperDirectory, "logs-snapshot.json");
        var originalJson =
            """
            {
              "lines": [ "old" ]
            ...
            """;
        await File.WriteAllTextAsync(logsSnapshotPath, originalJson);
        await using var lockedSnapshot = new FileStream(
            logsSnapshotPath,
            FileMode.Open,
            FileAccess.Read,
        ...
        using var service = new ManagementPersistenceService(
            ...
            new FakeLogsService { Snapshot = new ManagementLogsSnapshot { Lines = ["new"] } }
        );

        var exception = await Record.ExceptionAsync(() => service.SyncLogsSnapshotAsync());

        Assert.True(exception is IOException or UnauthorizedAccessException, exception?.ToString());
        Assert.Equal(originalJson, await File.ReadAllTextAsync(logsSnapshotPath));
        Assert.Empty(
            Directory.EnumerateFiles(keeperDirectory, "logs-snapshot.json.*.tmp")
        );
        Assert.NotNull(service.GetStatus().LastPersistenceError);
    }

    [Fact]
    public async Task ClearUsageSnapshotIgnoresLockedBackupCleanupFailure()
    {
        using var pathService = new TestPathService(_rootDirectory);
        var usageService = new FakeUsageService
        {
        ...
        var exception = await Record.ExceptionAsync(() => service.ClearUsageSnapshotAsync());
    ...
    [Fact]
    public async Task ClearUsageSnapshotIgnoresLockedLegacySnapshotDeleteFailure()
    {
        using var pathService = new TestPathService(_rootDirectory);
        await pathService.EnsureCreatedAsync();
        var legacySnapshotPath = Path.Combine(
    ...
    private sealed class FakeUsageService : IManagementUsageService
    {
        ...
        public Action<CancellationToken>? BeforeExportUsage { get; set; }
        ...
        {
            BeforeExportUsage?.Invoke(cancellationToken);
            cancellationToken.ThrowIfCancellationRequested();
