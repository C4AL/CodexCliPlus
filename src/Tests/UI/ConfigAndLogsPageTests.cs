...
public sealed class ConfigAndLogsPageTests
{
    ...
    public void ConfigPageUsesHeaderActionsInsteadOfBottomFloatingButtons()
    {
        ...
        var configPage = File.ReadAllText(
            Path.Combine(sourceRoot, "screens", "ConfigPage.tsx"),
            Encoding.UTF8
        );
        var configStyles = File.ReadAllText(
            Path.Combine(sourceRoot, "styles", "ConfigPage.module.scss"),
            Encoding.UTF8
        );
    ...
    public void CodexConfigPageOwnsRouteSwitchingAndUserCodexFileEditing()
    {
        ...
        var codexConfigPage = File.ReadAllText(
            Path.Combine(sourceRoot, "screens", "CodexConfigPage.tsx"),
            Encoding.UTF8
        );
        var dashboardOverviewPage = File.ReadAllText(
            Path.Combine(sourceRoot, "screens", "DashboardOverviewPage.tsx"),
            Encoding.UTF8
        );
        var sourceEditor = File.ReadAllText(
            Path.Combine(sourceRoot, "ui", "ConfigSourceEditor.tsx"),
            Encoding.UTF8
        );
        var diffModal = File.ReadAllText(
            Path.Combine(sourceRoot, "ui", "DiffModal.tsx"),
            Encoding.UTF8
        );
    ...
    public void AuthFilesImportExportUsesSacDesktopContract()
    {
        ...
        var authFilesStyles = File.ReadAllText(
            Path.Combine(sourceRoot, "styles", "AuthFilesPage.module.scss"),
            Encoding.UTF8
        );
        var zhCn = File.ReadAllText(
            Path.Combine(sourceRoot, "lib", "i18n", "locales", "zh-CN.json"),
            Encoding.UTF8
        );
