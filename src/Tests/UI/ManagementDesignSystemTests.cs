...
public sealed class ManagementDesignSystemTests
{
    ...
    public void AppProjectReferencesWebView2AndBuildToolPublishesGeneratedWebUiAssets()
    {
        ...
        Assert.DoesNotContain("assets\\webui\\upstream\\dist", csproj, StringComparison.Ordinal);
    ...
    [Fact]
    public void WebUiBuildAndDebugHostUsePackagedRepositoryDist()
    {
        var repositoryRoot = FindRepositoryRoot();
        ...
        Assert.Contains(
            "WebUiBuildDistRoot => Path.Combine(WebUiBuildSourceRoot, \"dist\")",
            buildTool,
            StringComparison.Ordinal
        );
        Assert.Contains(
    ...
    public void ManagementHomeAndTransitionLayersPreserveFullContentWidth()
    {
        ...
        var dashboardStyles = File.ReadAllText(
            Path.Combine(sourceRoot, "styles", "DashboardPage.module.scss"),
            Encoding.UTF8
        );
        var pageTransitionStyles = File.ReadAllText(
            Path.Combine(sourceRoot, "styles", "PageTransition.scss"),
            Encoding.UTF8
        );
    ...
    public void LegacyNativeManagementDesignSystemIsNotReferencedByRuntimeShell()
    {
        ...
        var appXaml = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "App.xaml"),
            Encoding.UTF8
        );
    ...
    public void LegacyNativeManagementPageLayerIsRemovedFromRepository()
    {
        ...
        string[] removedPaths =
        [
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "Views", "Pages"),
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "ViewModels", "Pages"),
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "Services", "SecondaryRoutes"),
            Path.Combine(repositoryRoot, "src", "DesktopShell", "CodexCliPlus.Management.DesignSystem"),
            Path.Combine(
                repositoryRoot,
                "src", "DesktopShell", "App",
                "Services",
    ...
    {
        var buildToolDirectory = Path.Combine(repositoryRoot, "src", "BuildPipeline", "BuildTool");
