...
public sealed class NavigationShellTests
{
    ...
    public void MainWindowHostsSingleWebViewAndTrayMenuContract()
    {
        ...
        var xaml = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "MainWindow.xaml"),
            Encoding.UTF8
        );
    ...
    public void WindowChromeAndStartupCardsAvoidTransparentShadowArtifacts()
    {
        ...
        var appProject = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "CodexCliPlus.App.csproj"),
            Encoding.UTF8
        );
        ...
        var updaterXaml = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "Updater", "MainWindow.xaml"),
            Encoding.UTF8
        );
        var startupFlowXaml = ReadStartupFlowXaml(repositoryRoot);
        var startupFlowResources = File.ReadAllText(
    ...
    public void AppSingleInstanceStartupWaitsOnlyWhenExistingInstanceIsExiting()
    {
        ...
        var source = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", "App.xaml.cs"),
            Encoding.UTF8
        );
    ...
    public void SettingsUpdateStatusUsesShortInlineMessages()
    {
        ...
        var hostSource = ReadMainWindowSources(repositoryRoot);
        var eventHandlersSource = ReadAppSource(
            repositoryRoot,
            "MainWindow.EventHandlers.cs"
        );
        var settingsUpdateSource = SliceBetween(
            eventHandlersSource,
            "private async void SettingsCheckUpdateButton_Click",
            "private async void SettingsApplyUpdateButton_Click"
        );
        var trayUpdateSource = SliceBetween(
            eventHandlersSource,
            "private async void TrayCheckUpdatesMenuItem_Click",
            "private static string BuildUpdateFailureMessage"
        );
        ...
        Assert.Contains("\"检查更新失败\"", settingsUpdateSource, StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            "BuildUpdateResultMessage(result)",
            trayUpdateSource,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain("result.Status", trayUpdateSource, StringComparison.Ordinal);
        Assert.DoesNotContain("result.Detail", trayUpdateSource, StringComparison.Ordinal);
        Assert.Contains("MessageBoxImage.Error", trayUpdateSource, StringComparison.Ordinal);
        AssertSourceOrder(
            trayUpdateSource,
            "if (!result.IsCheckSuccessful)",
            "var title = result.IsUpdateAvailable"
        );
        AssertSourceOrder(
            trayUpdateSource,
        ...
        Assert.Contains("当前未发现可用更新。", hostSource, StringComparison.Ordinal);
        Assert.Contains("当前仓库尚未发布稳定版本。", hostSource, StringComparison.Ordinal);
        Assert.Contains("未找到可直接应用的更新包", hostSource, StringComparison.Ordinal);
        Assert.Contains("发布页：{result.ReleasePageUrl}", hostSource, StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void FirstRunDesktopSaveUsesAtomicUtf8Writer()
    {
        var repositoryRoot = FindRepositoryRoot();
        var eventHandlersSource = ReadAppSource(repositoryRoot, "MainWindow.EventHandlers.cs");
        var saveHandlerSource = SliceBetween(
            eventHandlersSource,
            "private async void StartupFlow_FirstRunSaveRequested",
            "private async void StartupFlow_FirstRunConfirmationRequested"
        );

        Assert.Contains(
            "AtomicFileWriter.WriteUtf8NoBomTextAsync",
            saveHandlerSource,
            StringComparison.Ordinal
        );
        Assert.Contains("CancellationToken.None", saveHandlerSource, StringComparison.Ordinal);
        Assert.DoesNotContain("File.WriteAllTextAsync", saveHandlerSource, StringComparison.Ordinal);
        Assert.DoesNotContain("UTF8Encoding", saveHandlerSource, StringComparison.Ordinal);
    }
    ...
    public void DesktopWebUiRoutesLoginThroughNativeShell()
    {
        ...
        var dashboardOverviewPageSource = File.ReadAllText(
            Path.Combine(
                ...
                "src", "ManagementWeb",
                "src",
                "screens", "DashboardOverviewPage.tsx"
            ),
            Encoding.UTF8
        );
    ...
    [Fact]
    public void WebUiSourceContainsDesktopRecoveryBridge()
    {
        var repositoryRoot = FindRepositoryRoot();
        var bridgeSource = File.ReadAllText(
            Path.Combine(
    ...
    public void DashboardContainsLocalEnvironmentSnapshotAndRepairSurface()
    {
        ...
        Assert.Contains("LOCAL_ENVIRONMENT_AUTO_REFRESH_MS", dashboardSource, StringComparison.Ordinal);
        Assert.Contains("visibilitychange", dashboardSource, StringComparison.Ordinal);
        Assert.Contains("local-environment", dashboardSource, StringComparison.Ordinal);
        Assert.Contains("localDependencyRepairProgressTrack", dashboardSource, StringComparison.Ordinal);
        Assert.Contains("aria-busy", dashboardSource, StringComparison.Ordinal);
        Assert.DoesNotContain("response.result.succeeded ? 'success' : 'error'", dashboardSource, StringComparison.Ordinal);
        ...
        Assert.Contains("debugReportPath", bridgeSource, StringComparison.Ordinal);
        ...
        Assert.Contains("正在自动检测本机依赖", zhCn, StringComparison.Ordinal);
        Assert.Contains(
            "\"local_environment_required_repair\": \"一键修复\"",
            zhCn,
            StringComparison.Ordinal
        );
        Assert.Contains("已就绪 {{count}} 项", zhCn, StringComparison.Ordinal);
        Assert.Contains("日志路径", zhCn, StringComparison.Ordinal);
        Assert.Contains("修复入口仅在桌面模式可用", zhCn, StringComparison.Ordinal);
    }

    [Fact]
    public void WebUiMetadataPinsSourceCommit()
    {
        var repositoryRoot = FindRepositoryRoot();
    ...
    {
        var appDirectory = Path.Combine(repositoryRoot, "src", "DesktopShell", "App");
    ...
    private static string ReadAppSource(string repositoryRoot, string fileName)
    {
        return File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "DesktopShell", "App", fileName),
            Encoding.UTF8
        );
    }
