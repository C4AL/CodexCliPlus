...
public sealed class UiTextLocalizationTests
{
    ...
    public void DesktopWebUiChineseBrandingUsesCodexCliPlusAndBlocksBrowserManagementEntry()
    {
        ...
        var htmlEntry = File.ReadAllText(
            Path.Combine(repositoryRoot, "src", "ManagementWeb", "index.html"),
            Encoding.UTF8
        );
    ...
    public void DesktopWebUiKeepsOnlySimplifiedChineseLanguageSurface()
    {
        ...
        var constants = File.ReadAllText(
            Path.Combine(sourceRoot, "lib", "constants.ts"),
            Encoding.UTF8
        );
        var i18n = File.ReadAllText(Path.Combine(sourceRoot, "lib", "i18n", "index.ts"), Encoding.UTF8);
        var mainLayout = File.ReadAllText(
            Path.Combine(sourceRoot, "ui", "MainLayout.tsx"),
            Encoding.UTF8
        );
        ...
        Assert.False(File.Exists(Path.Combine(sourceRoot, "lib", "i18n", "locales", "en.json")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "lib", "i18n", "locales", "ru.json")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "lib", "i18n", "locales", "zh-TW.json")));
    }

    [Fact]
    public void DesktopWebUiSharedInputListFallbacksUseChineseText()
    {
        var repositoryRoot = FindRepositoryRoot();
        ...
        var headerInputList = File.ReadAllText(
            Path.Combine(componentRoot, "HeaderInputList.tsx"),
            Encoding.UTF8
        );
        var modelInputList = File.ReadAllText(
            Path.Combine(componentRoot, "ModelInputList.tsx"),
            Encoding.UTF8
        );
        var sharedInputs = string.Join(Environment.NewLine, headerInputList, modelInputList);

        Assert.Contains("keyPlaceholder = '请求头名称'", headerInputList, StringComparison.Ordinal);
        Assert.Contains("valuePlaceholder = '请求头值'", headerInputList, StringComparison.Ordinal);
        Assert.Contains("namePlaceholder = '模型名称'", modelInputList, StringComparison.Ordinal);
        Assert.Contains("aliasPlaceholder = '别名（可选）'", modelInputList, StringComparison.Ordinal);
        Assert.Contains("removeButtonTitle = '删除'", sharedInputs, StringComparison.Ordinal);
        Assert.Contains("removeButtonAriaLabel = '删除'", sharedInputs, StringComparison.Ordinal);
        Assert.DoesNotContain("removeButtonTitle = 'Remove'", sharedInputs, StringComparison.Ordinal);
        Assert.DoesNotContain("removeButtonAriaLabel = 'Remove'", sharedInputs, StringComparison.Ordinal);
        Assert.DoesNotContain("alias (optional)", sharedInputs, StringComparison.Ordinal);
        Assert.DoesNotContain("valuePlaceholder = 'value'", sharedInputs, StringComparison.Ordinal);
    }
    ...
    public void DesktopWebUiVisibleProviderSurfaceIsCodexOnly()
    {
        ...
        var routeSource = File.ReadAllText(
            Path.Combine(sourceRoot, "screens", "router", "MainRoutes.tsx"),
            Encoding.UTF8
        );
        var accountCenterPage = File.ReadAllText(
            Path.Combine(sourceRoot, "screens", "AccountCenterPage.tsx"),
            Encoding.UTF8
        );
        ...
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersGeminiEditPage.tsx"))
        );
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersClaudeEditPage.tsx"))
        );
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersOpenAIEditPage.tsx"))
        );
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersVertexEditPage.tsx"))
        );
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersAmpcodeEditPage.tsx"))
        );
        Assert.False(File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersPage.tsx")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "screens", "AiProvidersCodexEditPage.tsx")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "screens", "AuthFilesPage.tsx")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "screens", "QuotaPage.tsx")));
        Assert.False(File.Exists(Path.Combine(sourceRoot, "screens", "OAuthPage.tsx")));
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AuthFilesOAuthExcludedEditPage.tsx"))
        );
        Assert.False(
            File.Exists(Path.Combine(sourceRoot, "screens", "AuthFilesOAuthModelAliasEditPage.tsx"))
        );
