...
public sealed class UpdateInstallerServiceTests
{
    ...
    [Fact]
    public async Task DownloadInstallerAsyncRejectsUnsupportedDigestWithoutDeletingCachedPackage()
    {
        var installerBytes = Encoding.UTF8.GetBytes("cached-installer");
        using var pathService = new TestPathService();
        await pathService.EnsureCreatedAsync();
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.DownloadInstallerAsync(
                CreateUpdateResult("1.2.3", installerBytes.Length, "md5:abc123")
            )
        );

        Assert.Contains("Unsupported installer digest format", exception.Message);
        Assert.Equal(0, calls);
    ...
    [Fact]
    public async Task DownloadInstallerAsyncRejectsMalformedSha256DigestBeforeNetworkOrDiskUse()
    {
        using var pathService = new TestPathService();
        var calls = 0;
        using var factory = new FixedHttpClientFactory(_ =>
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.DownloadInstallerAsync(CreateUpdateResult("1.2.3", 0, "sha256:abc123"))
        );

        Assert.Contains("sha256:<64 hex>", exception.Message, StringComparison.Ordinal);
        Assert.Equal(0, calls);
        Assert.False(Directory.Exists(pathService.Directories.RootDirectory));
    }

    [Fact]
    public async Task DownloadInstallerAsyncIgnoresLockedStaleTemporaryFile()
    {
        var installerBytes = Encoding.UTF8.GetBytes("fresh-installer");
        const string assetName = "CodexCliPlus.Update.1.2.3.win-x64.zip";
        using var pathService = new TestPathService();
        await pathService.EnsureCreatedAsync();

        var updatesDirectory = Path.Combine(pathService.Directories.CacheDirectory, "updates");
        Directory.CreateDirectory(updatesDirectory);
        var staleTemporaryPath = Path.Combine(updatesDirectory, $"{assetName}.download");
        await File.WriteAllTextAsync(staleTemporaryPath, "stale");
        using var lockedStaleTemporary = new FileStream(
            staleTemporaryPath,
            FileMode.Open,
        ...
        Assert.True(result.DigestValidated);
        Assert.Equal(installerBytes, await File.ReadAllBytesAsync(result.InstallerPath));
        Assert.True(File.Exists(staleTemporaryPath));
    }

    [Fact]
    public void PromoteDownloadedInstallerKeepsExistingPackageWhenSourceMoveFails()
    {
        ...
        Directory.CreateDirectory(updatesDirectory);
        var installerPath = Path.Combine(updatesDirectory, "CodexCliPlus.Update.1.2.3.win-x64.zip");
        var temporaryPath = Path.Combine(updatesDirectory, "CodexCliPlus.Update.1.2.3.win-x64.zip.download");
        File.WriteAllText(installerPath, "cached-installer");
        File.WriteAllText(temporaryPath, "fresh-installer");
        using var lockedTemporary = new FileStream(
            temporaryPath,
            FileMode.Open,
            FileAccess.Read,
            FileShare.None
        ...
        var exception = Record.Exception(() =>
            UpdateInstallerService.PromoteDownloadedInstaller(temporaryPath, installerPath)
        );

        Assert.True(exception is IOException or UnauthorizedAccessException, exception?.ToString());
        Assert.Equal("cached-installer", File.ReadAllText(installerPath));
        Assert.True(File.Exists(temporaryPath));
    }

    [Fact]
    public async Task DownloadInstallerAsyncChoosesLargestFallbackUpdatePackage()
    {
        var smallBytes = Encoding.UTF8.GetBytes("small");
        var largeBytes = Encoding.UTF8.GetBytes("larger-installer");
        var smallAsset = new UpdateReleaseAsset
        {
            Name = "CodexCliPlus.Update.1.2.3.small.zip",
            DownloadUrl = "https://example.test/small.zip",
            Size = smallBytes.Length,
            Digest = ComputeDigest(smallBytes),
        };
        var largeAsset = new UpdateReleaseAsset
        {
            Name = "CodexCliPlus.Update.1.2.3.win-x64.zip",
            DownloadUrl = "https://example.test/large.zip",
            Size = largeBytes.Length,
            Digest = ComputeDigest(largeBytes),
        };
        using var pathService = new TestPathService();
        ...
        var result = await service.DownloadInstallerAsync(updateResult);

        Assert.Equal(largeAsset.Name, result.Asset.Name);
        Assert.Equal(
            Path.Combine(result.CacheDirectory, largeAsset.Name),
            result.InstallerPath
        );
        Assert.Equal(largeBytes, await File.ReadAllBytesAsync(result.InstallerPath));
    }
    ...
    [Fact]
    public async Task DownloadInstallerAsyncRejectsOfflineInstallerAssetBeforeNetworkOrDiskUse()
    {
        using var pathService = new TestPathService();
        var calls = 0;
        using var factory = new FixedHttpClientFactory(_ =>
        ...
        var updateResult = CreateUpdateResult(
            ...
            size: 0,
            digest: null,
            assetName: "CodexCliPlus.Setup.Offline.1.2.3.exe"
        );
    ...
    {
        var name = assetName ?? $"CodexCliPlus.Update.{version}.win-x64.zip";
        var asset = new UpdateReleaseAsset
        {
            Name = name,
            DownloadUrl = downloadUrl ?? $"https://example.test/{name}",
            Size = size,
            Digest = digest,
        };
