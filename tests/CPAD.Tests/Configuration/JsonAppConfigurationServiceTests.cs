...
public sealed class JsonAppConfigurationServicePathModeTests
{
        ...
        AppDataMode expectedMode)
    {
        ...
        Assert.Equal(CPAD.Core.Constants.AppConstants.DefaultBackendPort, actual.BackendPort);
    ...
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("cpad-config-remember");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);
    ...
    {
        using var scope = new AppPathServiceEnvironmentScope();
        var overrideRoot = scope.CreateTemporaryRoot("cpad-config-legacy-secret");
        AppPathServiceEnvironmentScope.SetRootOverride(overrideRoot);

        var pathService = new AppPathService();
        var store = new CPAD.Infrastructure.Security.DpapiCredentialStore(pathService);
