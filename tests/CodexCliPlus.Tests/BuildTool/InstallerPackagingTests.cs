...
{
    private readonly string _rootDirectory = Path.Combine(Path.GetTempPath(), $"codexcliplus-installer-tests-{Guid.NewGuid():N}");
    ...
    public async Task PackageInstallerBuildsExecutableAndStagingArchiveFromRepoOwnedToolchain()
    {
        ...
        var cleanupManifest = ReadArchiveJson<InstallerCleanupManifest>(archive, "app-package/packaging/uninstall-cleanup.json");
    ...
    {
        var toolchainRoot = Path.Combine(repositoryRoot, "build", "micasetup", "toolchain", "build");
        ...
        File.WriteAllText(
            Path.Combine(repositoryRoot, "build", "micasetup", "toolchain", "micasetup-tools-version.txt"),
            "repo-owned-test");
    }

    private static T ReadArchiveJson<T>(ZipArchive archive, string entryName)
    {
        var entry = archive.GetEntry(entryName) ?? throw new Xunit.Sdk.XunitException($"Missing zip entry: {entryName}");
    ...
    private sealed class InstallerProcessRunner(bool forceFallback) : IProcessRunner
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                if (arguments.Count > 0 && string.Equals(arguments[0], "a", StringComparison.OrdinalIgnoreCase))
                {
                    File.WriteAllBytes(arguments[1], [1, 2, 3, 4]);
                    return Task.FromResult(0);
                }

                if (arguments.Count > 0 && string.Equals(arguments[0], "x", StringComparison.OrdinalIgnoreCase))
                {
                    var outputArgument = arguments.Single(arg => arg.StartsWith("-o", StringComparison.Ordinal));
            ...
            if (fileName.EndsWith("makemica.exe", StringComparison.OrdinalIgnoreCase))
            {
                ...
                File.WriteAllBytes(outputPath, forceFallback ? Encoding.UTF8.GetBytes("bad") : CreateExecutableBytes());
                return Task.FromResult(0);
            }
