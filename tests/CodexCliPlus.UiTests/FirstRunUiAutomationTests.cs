...
public sealed class FirstRunUiAutomationTests
{
    ...
    {
        using var run = UiTestRun.Start(
            nameof(FirstRunProbeCapturesNativeUiArtifactsForManualReview)
        );
        ...
        AssertFirstRunKeyRowGeometry(
            keyBoxBounds,
            saveButtonBounds,
            copyButtonBounds,
            rememberCheckBoxBounds
        );
        ...
        var savedFiles = run.WaitFor(
            () =>
                run.GetNewDesktopSecurityKeyFiles().Length > 0
                    ? run.GetNewDesktopSecurityKeyFiles()
                    : null,
        ...
        run.WriteJson(
            "desktop-save.json",
            new { createdFiles = savedFiles.Select(Path.GetFileName).ToArray() }
        );
        ...
        run.CaptureElement(
            ...
            "copy-notification-message-crop.png",
            padding: 12
        );
        ...
        run.CaptureElement(
            "FirstRunConfirmCloseButton",
            "confirm-close-button-crop.png",
            padding: 10
        );
    ...
    {
        using var run = UiTestRun.Start(
            nameof(FirstRunProbeCapturesUnrememberedLoginFlowForManualReview)
        );
        ...
        var continueButton = run.WaitFor(
            () =>
            {
                ...
                return element is not null && element.Properties.IsEnabled.ValueOrDefault
                    ? element
                    : null;
            },
        ...
        run.CaptureElement(
            "FirstRunConfirmContinueButton",
            "confirm-ready-button-crop.png",
            padding: 10
        );
        ...
        run.CaptureElement(
            "LoginSilentLoginRiskIndicator",
            "login-silent-risk-crop.png",
            padding: 8
        );
        run.WriteUiaTree("uia-tree.json");
        run.WriteReview(
        ...
        Assert.True(
            File.Exists(Path.Combine(run.ArtifactDirectory, "unremembered-login-full.png"))
        );
    }
    ...
    {
        using var run = UiTestRun.Start(
            nameof(FirstRunProbeCapturesRememberedWebUiBridgeForManualReview)
        );
    ...
    private static async Task<WebUiProbeState> CaptureWebUiStateAsync(IPage page, UiTestRun run)
    {
        var state = new WebUiProbeState(
            ...
            await page.EvaluateAsync<bool>("Boolean(document.querySelector('.app-shell'))"),
            await page.EvaluateAsync<bool>(
                "Boolean(document.body?.innerText?.includes('桌面登录已失效'))"
            ),
            await page.EvaluateAsync<string>("(document.body?.innerText || '').slice(0, 500)")
        );
    ...
    )
    {
        ...
        Assert.True(
            saveButton.Width >= minimumButtonSide,
            $"保存按钮宽度异常：{saveButton.Width:0.##}"
        );
        Assert.True(
            saveButton.Height >= minimumButtonSide,
            $"保存按钮高度异常：{saveButton.Height:0.##}"
        );
        Assert.True(
            copyButton.Width >= minimumButtonSide,
            $"复制按钮宽度异常：{copyButton.Width:0.##}"
        );
        Assert.True(
            copyButton.Height >= minimumButtonSide,
            $"复制按钮高度异常：{copyButton.Height:0.##}"
        );
        ...
        Assert.True(
            copyButton.Right <= keyBox.Right + tolerance,
            "复制按钮必须悬浮在安全密钥框右侧内部。"
        );
        Assert.True(
            keyBox.Right <= saveButton.Left + tolerance,
            "保存按钮必须位于安全密钥框外侧。"
        );
        Assert.True(
            copyButton.Right <= saveButton.Left + tolerance,
            "复制按钮和保存按钮不能重叠。"
        );
        Assert.True(
            saveButton.Right <= rememberCheckBox.Left + tolerance,
            "保存按钮必须位于记住安全密钥选项左侧。"
        );
    }
    ...
    private static async Task<IPage> WaitForWebViewPageAsync(IBrowser browser, TimeSpan timeout)
    {
        ...
        {
            var page = browser
                .Contexts.SelectMany(context => context.Pages)
                .FirstOrDefault(candidate =>
                    candidate.Url.Contains(
                        "codexcliplus-webui.local",
    ...
    private static async Task<IBrowser> WaitForCdpBrowserAsync(
    ...
    )
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            try
            {
                ...
                when (exception is PlaywrightException or HttpRequestException)
            {
                lastError = exception;
                await Task.Delay(300);
            }
